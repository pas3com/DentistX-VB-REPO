...
#Region "PatientSearch"
    ...
    Private _lastCommittedSearchText As String
    Private _lastCommittedSearchPatients As List(Of Patient)
    Private _lastCommittedSuggestionPatientName As String

    ''' <summary>After a deliberate "park" (empty <see cref="PatientBS"/>), the debounced search timer must not
    ''' repopulate the list until the user types a non-empty query or selects a patient — otherwise first-load
    ''' "no selection" is undone on the next tick.</summary>
    Private _parkBindingEmptyUntilUserQuery As Boolean
    ...
    ''' <summary>Rebuild <see cref="_filteredPatients"/> from <see cref="_allPatients"/> for current <see cref="_Target"/>.</summary>
    Private Sub RebuildFilteredPatientsFromAll()
        If _allPatients Is Nothing Then
            _filteredPatients = New List(Of Patient)()
            Return
        End If
    ...
    Private Sub UpdateParkIntentAfterRefresh(parkNoCurrentPatient As Boolean)
        If parkNoCurrentPatient AndAlso _bindingPatients IsNot Nothing AndAlso _bindingPatients.Count = 0 Then
            _parkBindingEmptyUntilUserQuery = True
        ElseIf _bindingPatients IsNot Nothing AndAlso _bindingPatients.Count > 0 Then
            _parkBindingEmptyUntilUserQuery = False
        End If
    End Sub
    ...
    ''' <param name="suppressPatientBroadcast">When True with empty search, suppress PatientChanged during list refresh (caller fires once after).</param>
    ''' <param name="selectPatientIdAfterRefresh">When set, select this patient after rebuilding the binding list.</param>
    Private Sub ApplyFilter(Optional parkNoCurrentPatient As Boolean = False, Optional suppressPatientBroadcast As Boolean = False, Optional selectPatientIdAfterRefresh As Integer? = Nothing)
        ...
        RebuildFilteredPatientsFromAll()
    ...
    Private Sub RefreshBindingListFromFiltered(Optional searchText As String = Nothing, Optional suppressPatientBroadcast As Boolean = False, Optional parkNoCurrentPatient As Boolean = False, Optional selectPatientIdAfterRefresh As Integer? = Nothing)
        ...
        End Try
        UpdateParkIntentAfterRefresh(parkNoCurrentPatient)
    ...
    Private Sub ClearCommittedSuggestionSearchContext()
        _lastCommittedSearchText = Nothing
        _lastCommittedSearchPatients = Nothing
        _lastCommittedSuggestionPatientName = Nothing
    End Sub
    ...
    Private Function ShouldUseCommittedSuggestionSearchContext(forceShow As Boolean) As Boolean
        ...
        If _lastCommittedSearchPatients Is Nothing OrElse _lastCommittedSearchPatients.Count = 0 Then Return False
        If txtPatientName Is Nothing Then Return False

        Dim currentText = NormalizePatientSearchText(If(txtPatientName.Text, "").Trim())
        Dim committedPatientName = NormalizePatientSearchText(If(_lastCommittedSuggestionPatientName, "").Trim())
    ...
    Public Sub UpdateFilterTarget(filterTarget As String, Optional passedPatient As Patient = Nothing) Implements IPatientHeaderControl.UpdateFilterTarget
        ...
        If IsMajorNavigatorFilterTarget(requestedTarget) Then
            ...
            If Not (isSameMajorTarget AndAlso hasActiveQuery) Then
                ' Without parking, ApplyFilter() fills the binding list and BindingSource selects index 0 — so the *first*
                ' switch after load could look correct (still empty if timing/race) but *second* switch (e.g. Treat→Ortho)
                ' always picked the first patient. Match initial-load behavior: new-header mode + empty search → stay parked.
    ...
    Private Sub OnSearchTimerTick(sender As Object, e As EventArgs)
        _searchTimer.Stop()
        Dim raw = If(txtPatientName Is Nothing, "", txtPatientName.Text).Trim()
        If _parkBindingEmptyUntilUserQuery AndAlso raw.Length = 0 Then
            ...
            Return
        End If
        If raw.Length > 0 Then _parkBindingEmptyUntilUserQuery = False
    ...
    Private Sub UpdateSearchResultsButtonVisibility()
        ...
        If ShouldUseCommittedSuggestionSearchContext(True) Then
            hasResults = _lastCommittedSearchPatients.Count > 0
        Else
    ...
    Private Sub UpdateAndShowPatientSuggestions(Optional forceShow As Boolean = False)
        ...
        If ShouldUseCommittedSuggestionSearchContext(forceShow) Then
            q = If(_lastCommittedSearchText, "").Trim()
            displayList = New List(Of Patient)(_lastCommittedSearchPatients)
        Else
    ...
    Private Sub CommitSuggestionAt(index As Integer)
        ...
        Dim searchText = If(useCommittedContext, If(_lastCommittedSearchText, ""), If(txtPatientName Is Nothing, "", txtPatientName.Text))
        ...
        _lastCommittedSearchPatients = searchPatients
        _lastCommittedSuggestionPatientName = If(p.PatientName, "")
        _suppressSearchText = True
        Try
...
#Region "Initialize"
    ...
    Private Sub TeardownAuxiliaryTimers()
        If _searchTimer IsNot Nothing Then
            Try
                _searchTimer.Stop()
                RemoveHandler _searchTimer.Tick, AddressOf OnSearchTimerTick
            Catch
            End Try
            Try
            ...
            Sub(t)
                ...
                BeginInvoke(Sub()
                                ...
                                If t.IsFaulted Then
                                    ...
                                    Debug.WriteLine("StartNavigator3PatientsBackgroundLoad failed: " & If(ex?.Message, ""))
                                    Return
                                End If
                                If t.Result Is Nothing Then Return
                                _allPatients.Clear()
...
#Region "PatientHdrControls"
    ...
    Private Function GetTypedNameForNewPatient() As String
        ...
        Dim normalizedTyped As String = NormalizePatientSearchText(typedName)
        Dim exactMatchSavedName As Boolean =
            _allPatients IsNot Nothing AndAlso
            _allPatients.Any(Function(p) String.Equals(
                NormalizePatientSearchText(If(p.PatientName, "")),
                normalizedTyped,
    ...
    Private Sub ShowFly(Optional focusPatientNameEditWhenShown As Boolean = False,
        ...
        BeginInvoke(New Action(
            Sub()
                If Me.IsDisposed OrElse FlyoutPatientInfo Is Nothing OrElse FlyoutPatientInfo.IsDisposed Then Return
                Try
                ...
                Catch ex As ObjectDisposedException
                    Debug.WriteLine("Navigator3 ShowFly ObjectDisposed: " & ex.Message)
                Catch ex As Exception
                    Debug.WriteLine("Navigator3 ShowFly: " & ex.Message)
                End Try
            End Sub))
    End Sub
    ...
    ''' </summary>
    Private Sub HidePatientFlyoutSafe()
        Try
            If FlyoutPatientInfo Is Nothing OrElse FlyoutPatientInfo.IsDisposed Then Return
    ...
    Private Sub FlyoutPatientInfo_Showing(sender As Object, e As FlyoutPanelEventArgs) Handles FlyoutPatientInfo.Showing
    ...
    Private Sub btNewPatient_Click(sender As Object, e As EventArgs) Handles btNewPatient.Click
        ...
        RestoreNewPatientFlyoutFromSearch()
        ShowFly(focusPatientNameEditWhenShown:=True)
    End Sub
    ...
    Private Sub OpenPopupPatientFlyout()
        Dim focusNameFieldAfterShown As Boolean = False
        If _popupOpeningForAdd Then
            RestoreNewPatientFlyoutFromSearch()
            focusNameFieldAfterShown = True
        Else
        ...
        End If
        _popupOpeningForAdd = False
        ShowFly(focusPatientNameEditWhenShown:=focusNameFieldAfterShown, ownerAnchor:=PopupPatient)
    End Sub
...
#Region "PatientAddUpdateDeleteControls"
    ...
    Private Sub InsertPatient()
        ...
        RebuildFilteredPatientsFromAll()
        Dim matchesFilter = _filteredPatients.Exists(Function(p) p.PatientID = newPatient.PatientID)
        If matchesFilter Then
            _bindingPatients.Add(newPatient)
            Dim newIndex As Integer = _bindingPatients.Count - 1
            PatientBS.Position = newIndex
        End If
        ' After add: clear stale search safely, resync _filteredPatients from _allPatients, keep correct row selected;
        ' suppress PatientChanged until InsertPatient ends with FirePatientChangedForCurrent below.
        ...
        End If
        If matchesFilter Then _parkBindingEmptyUntilUserQuery = False
        ' Add flow left this True; without clearing, PopupPatient_Click treats next open as Add and clears fields.
        _popupOpeningForAdd = False
...
#Region "Resize"
    ...
    Private Sub OnResizeCoalesceTick(sender As Object, e As EventArgs)
        ...
        Try
            ResizeControlsProportionally()
        Catch ex As Exception
            Debug.WriteLine("Navigator3 proportional resize: " & ex.Message)
        End Try
    End Sub
#End Region
...
#Region "GetImgs"
    ...
    ''' <summary>User came back to the search box by Tab or programmatic focus. Keep the caret stable; deferred SelectAll on DevExpress TextEdit can eat the first typed character.</summary>
    ...
    Private Sub txtPatientName_ClickSelectAll(sender As Object, e As EventArgs) Handles txtPatientName.Click
        If _ignoreSearchSelectAllOnce Then Return
