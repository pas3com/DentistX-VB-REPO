...
Public Class AdultDiag
    ...
    Private Function SafeString(row As DataRow, columnName As String, Optional defaultValue As String = "") As String
        If row Is Nothing OrElse row.IsNull(columnName) Then Return defaultValue
        Return row(columnName).ToString()
    End Function

    Private Function SafeInt(row As DataRow, columnName As String, Optional defaultValue As Integer = 0) As Integer
        If row Is Nothing OrElse row.IsNull(columnName) Then Return defaultValue
        Dim value = row.Field(Of Integer?)(columnName)
        Return value.GetValueOrDefault(defaultValue)
    End Function


    'Private Sub AdultDiag_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "The Class"
    ...
    Public Sub New()
        ...
        'LoadPatientTreats(PatientID)
        LogTime("AdultDiag_New", Me.Name, sw)
    End Sub
    ...
    Public Property ValueFromParent As Integer
        ...
        End Set
    End Property
    ' --- Diagnosis adult jaw (experiment scope): mobile diag rows from dbo.Patient_Diagnosis LVL=9 (GetPatientTeethMobDiags / GetPatientToothMobDiags). ---
    ' --- Paired SlctdToothCTLDiag Source="Mobile" now uses that table instead of Patient_Mobile; see AdultDiag SetAddedMobsListDataSource (already LVL=9). ---
...
#Region "MouseActivity"
    ...
    Private Sub OnBeforeMaintenanceModalFromOtherControls(sender As Object, e As EventArgs)
        If FlyMenu IsNot Nothing AndAlso FlyMenu.Visible Then FlyMenu.Visible = False
    End Sub
...
#Region "Svg Item Events"
    ...
    Private Sub CommonSvgItemEnter(sender As Object, e As SvgImageItemEventArgs)
        ...
        Dim toothTrts As IEnumerable(Of Patient_Diagnosis) = clsToothTrtData.Select_BypID_tNum(New Patient_Diagnosis With {.PatientID = PatientID, .ToothNum = toothNum})
        Dim notesList As New List(Of String)()
        If toothTrts IsNot Nothing Then
            Dim itemId As String = If(svgItem?.Id, "")
            For Each t As Patient_Diagnosis In toothTrts
...
#End Region
    ...
    Private Sub ShowTreatmentFlyoutForSvgRightClick(svg As SvgImageBox)
        ...
        JawFlyMenuRuntime.PositionFlyMenuForTooth(FlyMenu, svg, StringComparison.Ordinal)
    ...
    Private Sub CommonMouseHoverHandler(sender As Object, e As EventArgs)
        ...
        Dim titleItem As New ToolTipTitleItem()
        Dim baseNameOld As String = Svg.Name.Substring(0, Svg.Name.Length - 1)
        titleItem.Text = $"{baseNameOld} -->> {Svg.Tag.ToString()}"
        ...
        Dim Trts As List(Of String) = GetAddedTreatments(PatientID, toothNum, GetToothFullName(toothName))
...
#Region "New TreeViewSource"
    ...
    ' In-memory master list (full set for the mobile page) — uses TrtSourceHelper.TrtNodeInfo
    ...
    Private Sub BuildTreeFromList_Old(nodeList As List(Of TrtSourceHelper.TrtNodeInfo), Optional existingTrts As HashSet(Of Integer) = Nothing)
        ...
        Try
            ...
            ' Create DIAGNOSES as the root node
            Dim diagnosesRoot As New TreeNode("DIAGNOSES") With {
            .Name = "DIAGNOSES",
            .ForeColor = Color.DarkRed,
            .Tag = "ROOT_PARENT",
            .NodeFont = New Font(TrtsTreeView.Font, FontStyle.Bold)
        }
            ...
            For Each Inf In nodeList
                ...
                If parent.ToUpperInvariant() = "IMPLANT" Then
                    ...
                    ' Ensure IMPLANT parent node exists under DIAGNOSES
                    If Not parentNodes.ContainsKey(parent) Then
                        ...
                        ' Add IMPLANT under DIAGNOSES root
                        diagnosesRoot.Nodes.Add(pnode)
                    End If
                ...
                Else
                    ...
                    If Not String.IsNullOrEmpty(parent) Then
                        If Not parentNodes.ContainsKey(parent) Then
                            ...
                            parentNodes.Add(parent, pnode)
                            ' Add parent group under DIAGNOSES root
                            diagnosesRoot.Nodes.Add(pnode)
                        End If
                    End If
                    ...
                    Else
                        ' For treatments without a parent group, add their group directly under DIAGNOSES
                        ' and add treatment under that group
                        If groupNodes(grp).Parent Is Nothing Then
                            diagnosesRoot.Nodes.Add(groupNodes(grp))
                        End If
                        groupNodes(grp).Nodes.Add(tnode)
                    End If
                End If
            ...
            ' Add DIAGNOSES root to TreeView (always add it, even if empty)
            TrtsTreeView.Nodes.Add(diagnosesRoot)

            ' Add any orphan groups that don't have parents and weren't added yet
            For Each kvp In groupNodes
                If kvp.Value.Parent Is Nothing Then
                    diagnosesRoot.Nodes.Add(kvp.Value)
                End If
            Next

            ' Expand DIAGNOSES by default
            diagnosesRoot.Expand()

            ' collapse parent groups if you want them collapsed by default
            For Each n As TreeNode In diagnosesRoot.Nodes
            ...
            ' If empty, show info
            If diagnosesRoot.Nodes.Count = 0 Then
                diagnosesRoot.Nodes.Add(New TreeNode("No available treatments for current tooth status") With {.ForeColor = Color.Gray, .Tag = "INFO_NODE"})
            End If
        Finally
...
#End Region
    ...
    Private Sub FilterMobsTreeView(searchText As String)
        ...
        Using connection As New SqlConnection(connectionString)
            ...
            Where(Function(row) SafeString(row, "Trt").ToLower().Contains(searchText)).
            GroupBy(Function(row) SafeString(row, "TrtGroup"))

            ' Add groups and treatments to TreeView
            For Each group In groups
                Dim groupNode As New TreeNode(group.Key)
                For Each treatment In group
                    Dim treatmentNode As New TreeNode(SafeString(treatment, "Trt")) With {
                    .Tag = SafeInt(treatment, "TrtID")
                }
...
#Region "Loading"
    ...
    Private Sub AdultDiag_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'sw.Stop()
        'LogToFile("AdultDiag_Load Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
        LogTime(NameOf(AdultDiag_Load), Me.Name, sw)
    End Sub
    ...
    Private Sub AdultDiag_Resize(sender As Object, e As EventArgs) Handles Me.Resize
        If controlBoundsCache.IsEmpty Then Return
        ResizeControlsProportionally()
        ALIGNS()
    End Sub
...
#Region "FlyOutEvents"
    ...
    Private NotInheritable Class FlyMenuOutsideClickFilter
        ...
        Private ReadOnly _jaw As AdultDiag

        Public Sub New(jaw As AdultDiag)
            _jaw = jaw
        End Sub
    ...
    ''' <summary>Patient resolution: workspace patient → DiagUserControl host → FormManager fallback, then open <see cref="DelDiagForm"/> for the single/multi/whole-mouth target list.</summary>
    ...
    ''' <summary>Same patient + tooth-list resolution as <see cref="btnDelTrts_Click"/>, then opens <see cref="EditMultiTreatFrom"/>. Hidden on Diag jaws via <c>hideEditMulti:=True</c>, kept here so the Handles binding still resolves.</summary>
...
#Region "Svg Item Events"
    ...
    Private Sub zSvg_ItemEnter(sender As Object, e As SvgImageItemEventArgs) Handles zSvg.ItemEnter
        ...
        ' Create a SuperToolTipTitleItem for the title
        Dim titleItem As New ToolTipTitleItem()
        Dim baseNameOld As String = zSvg.Name.Substring(0, zSvg.Name.Length - 1)
        titleItem.Text = $"{baseNameOld} -->> {zSvg.Tag.ToString()}"
