...
#Region "Week view render performance (toggles + legacy revert note)"
    ...
    ''' <summary>
    ''' Max <see cref="HtmlCard"/> rows per day column. 0 = unlimited (slower for busy days &amp; on maximize). Default 40 keeps UI responsive; open day header for full list.
    ''' </summary>
    ...
    Private Const WeekDayDrawerUseDimOverlay As Boolean = False
#End Region
...
#End Region
    ...
    Private _weekDragCard As HtmlCard
    ...
    Private Const WeekDayDrawerWidthPx As Integer = 140
    ...
    Friend Sub WireDrawerDayCard(card As HtmlCard, dayDate As DateTime)
    ...
    Private Sub PopulateDayCards(cardsLayer As PanelControl, dayDate As DateTime, state As ApptState, data As ApptDataBundle)
        ...
        Try
            ...
            For Each appointment In dayAppointments
                ...
                card.ApplyContentHeightToFitForWeekView()
                AddHandler card.AppointmentClicked, Sub(ap) If InteractionHub IsNot Nothing Then InteractionHub.PublishAppointmentClicked(ap)
                AddHandler card.AppointmentDoubleClicked, Sub(ap) If InteractionHub IsNot Nothing Then InteractionHub.PublishAppointmentDoubleClicked(ap)
                AddHandler card.StatusContextEditRequested, Sub(ap) If InteractionHub IsNot Nothing Then InteractionHub.PublishAppointmentDoubleClicked(ap)
                AddHandler card.StatusChangeRequested,
    ...
    ''' <param name="remeasureUnchangedWidth">When false, <see cref="HtmlCard.ApplyContentHeightToFitForWeekView"/> runs only if the card width actually changed (resize / scrollbar toggle). Use true after data bind or snapshot.</param>
    Private Sub LayoutDayCards(cardsLayer As PanelControl, Optional remeasureUnchangedWidth As Boolean = True)
        ...
        For Each ctrl As Control In cardsLayer.Controls.OfType(Of Control)().Reverse()
            ...
            ctrl.Width = width
            Dim weekCard = TryCast(ctrl, HtmlCard)
    ...
    Private Sub AdjustDayScrollExtentAt(columnIndex As Integer)
        ...
        Try
            ...
            End If
        Finally
            ' True: one layout pass for HtmlCard children after width/height adjustments.
            cardsLayer.ResumeLayout(True)
        End Try
    ...
    Private Sub OnWeekCardDragMouseDown(card As HtmlCard, sourceDay As Date, e As MouseEventArgs)
        If e.Button <> MouseButtons.Left Then Return
    ...
    Private Sub OnWeekCardDragMouseUp(card As HtmlCard, e As MouseEventArgs)
        _weekDragTimer.Stop()
    ...
    Private Sub StartWeekCardDrag(card As HtmlCard, sourceDay As Date)
        Try
    ...
    Private Sub TryScrollToAppointment(ap As AppointmentC)
        ...
        Dim cardCtl = cardsLayer.Controls.OfType(Of HtmlCard)().FirstOrDefault(
        ...
        If cardCtl Is Nothing Then
            cardCtl = cardsLayer.Controls.OfType(Of HtmlCard)().FirstOrDefault(
    ...
    Friend Function BuildHtmlExportContext(weekCaption As String) As WeekSnapshotHtmlContext
        ...
        For i = 0 To _dayHosts.Count - 1
            ...
            For Each c In sc.Cards.Controls.OfType(Of HtmlCard)().OrderBy(Function(x) x.Top)
...
#Region "Week day drawer (HtmlCard list)"
...
Friend NotInheritable Class WeekDayCardDrawerHost
    ...
    Private Sub RelayoutDrawerCards()
        ...
        For Each c As Control In _scroll.Controls
            c.Width = w
            Dim weekCard = TryCast(c, HtmlCard)
