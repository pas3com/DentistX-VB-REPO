...
Public Class ApptDayLine
    ...
    ''' <summary>Width of the day-name column on the left; shared with HTML/PNG export so layout matches.</summary>
    Friend Const DayLabelWidth As Integer = 120
    ''' <summary>Minimum horizontal span for one clock hour on the time axis (wider = roomier appointment chips). When this × visible hours exceeds the viewport, the host scrolls horizontally. Shared with HTML export.</summary>
    Friend Const MinTimelinePixelsPerHour As Integer = 210 '92
    ...
    Private Sub RenderTimeline()
        ...
        Try
            ...
            If _snapshotScrollInnerWCap >= 0 Then
                innerW = Math.Min(innerW, Math.Max(200, _snapshotScrollInnerWCap))
            End If
            _lastScrollInnerW = innerW
            Dim timelineWidthFromView = Math.Max(120, innerW - DayLabelWidth - 2)
            Dim timelineWidthFromMinScale = CInt(Math.Ceiling((CDbl(totalTimeMinutes) / 60.0R) * MinTimelinePixelsPerHour))
            Dim timelineWidth = Math.Max(timelineWidthFromView, timelineWidthFromMinScale)
