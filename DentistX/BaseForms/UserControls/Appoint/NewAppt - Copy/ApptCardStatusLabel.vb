...
Public Class ApptCardStatusLabel
    ...
    ''' <summary>
    ''' Minimum <see cref="Control.Height"/> so rotated vertical status text stays on one line (matches <see cref="DrawTextRotatedPath"/> em-size clamp).
    ''' <paramref name="stripControlWidth"/> is the strip control width in pixels (e.g. <see cref="Control.Width"/> when docked right).
    ''' </summary>
    Public Shared Function MeasureMinimumStripHeight(statusText As String, font As Font, stripControlWidth As Integer, Optional dpiY As Single = 96.0F) As Integer
        If String.IsNullOrWhiteSpace(statusText) OrElse font Is Nothing Then Return 1
        Try
            If font.FontFamily Is Nothing Then Return 1
        Catch
            Return 1
        End Try
        Dim drawText = statusText.Trim()
        Const pad As Single = 2.0F
        Dim innerRW = Math.Max(1.0F, CSng(Math.Max(1, stripControlWidth)) - 2.0F * pad)
        Dim layoutH = innerRW
        Dim emSize = CSng(font.Size * dpiY / 72.0F)
        Dim maxEm = layoutH * 0.85F
        If maxEm > 0.0F AndAlso emSize > maxEm Then emSize = maxEm
        ...
        Using fmt As New StringFormat() With {.Alignment = StringAlignment.Center, .LineAlignment = StringAlignment.Center}
            fmt.FormatFlags = StringFormatFlags.LineLimit Or StringFormatFlags.NoWrap
            Using path As New GraphicsPath()
                path.AddString(drawText, font.FontFamily, CInt(font.Style), emSize, New RectangleF(0, 0, 32000.0F, layoutH), fmt)
                Dim b = path.GetBounds()
                Dim extent = Math.Max(b.Width, 1.0F)
                Dim innerRh = extent + 2.0F * pad + 4.0F
                Return Math.Max(1, CInt(Math.Ceiling(innerRh + 2.0F * pad)))
            End Using
        End Using
    End Function
