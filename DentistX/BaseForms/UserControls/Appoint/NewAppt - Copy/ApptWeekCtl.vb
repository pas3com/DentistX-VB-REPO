...
#Region "Week view render performance (toggles + legacy revert note)"
    ...
    Private Const WeekViewResizeDebounceMs As Integer = 62
    Private Const WeekViewResizeDebounceMaxMs As Integer = 108
...
#End Region
    ...
    Private _dialogExpandedFullWeek As FullWeekSched
    Private _dialogExpandedFullWeekTemporarily As Boolean
    ...
    Protected Overrides Sub Dispose(disposing As Boolean)
        If disposing Then
            Try
                ...
                _dialogExpandedFullWeek = Nothing
                _dialogExpandedFullWeekTemporarily = False
    ...
    ''' <summary>
    ''' Matches <see cref="ApptTheme.OrderAppointmentsForWeekDayGroupsAndSolos"/> (group blocks vs solo appts, merged by time).
    ''' Used when laying out controls: z-order / <c>BringToFront</c> must not invert the visual stack.
    ''' </summary>
    Friend Shared Function OrderVisibleWeekApptCardsForLayout(cards As IEnumerable(Of ApptCard), data As ApptDataBundle) As List(Of ApptCard)
        Dim list = cards.
            Where(Function(c) c IsNot Nothing AndAlso c.Visible AndAlso c.BoundAppointment IsNot Nothing).
            ToList()
        If list.Count = 0 Then Return list
        Dim orderedAppts = ApptTheme.OrderAppointmentsForWeekDayGroupsAndSolos(list.Select(Function(c) c.BoundAppointment), data)
        Dim result As New List(Of ApptCard)
        Dim unmatched As New List(Of ApptCard)(list)
        For Each ap In orderedAppts
            Dim card = unmatched.FirstOrDefault(Function(c) WeekApptEqualsForLayout(c.BoundAppointment, ap))
        ...
        If unmatched.Count > 0 Then
            For Each ap In ApptTheme.OrderAppointmentsForWeekDayGroupsAndSolos(unmatched.Select(Function(c) c.BoundAppointment), data)
                Dim card = unmatched.FirstOrDefault(Function(c) WeekApptEqualsForLayout(c.BoundAppointment, ap))
                If card IsNot Nothing Then
                    result.Add(card)
                    unmatched.Remove(card)
    ...
    Private Shared Function WeekApptEqualsForLayout(a As AppointmentC, b As AppointmentC) As Boolean
        If a Is Nothing OrElse b Is Nothing Then Return False
        If a.AppointmentID > 0 AndAlso b.AppointmentID > 0 Then Return a.AppointmentID = b.AppointmentID
        Return a.StartDateTime = b.StartDateTime AndAlso a.PatientID = b.PatientID
    End Function
    ...
    Private Sub LayoutDayCards(cardsLayer As PanelControl, Optional remeasureUnchangedWidth As Boolean = True)
        ...
        Dim weekApptCardsAll = cardsLayer.Controls.OfType(Of ApptCard)().Where(Function(ctrl) ctrl.Visible).ToList()
        Dim weekApptCards = weekApptCardsAll.Where(Function(c) c.BoundAppointment IsNot Nothing).ToList()
        Dim visibleCards = OrderVisibleWeekApptCardsForLayout(weekApptCards, If(_request?.Data, Nothing)).
            Cast(Of Control)().
            Concat(weekApptCardsAll.Where(Function(c) c.BoundAppointment Is Nothing)).
            ToList()
    ...
    ''' <summary>
    ''' Largest <c>VScrollBar.Value</c> that keeps the bottom of <paramref name="cardsLayer"/> at the viewport bottom (no empty tail).
    ...
    ''' <summary>Refreshes day-column scroll extents after host navigation.</summary>
    ...
    Private Sub PrepareDialogOverlayParent()
        ...
        Dim fullWeek = If(host Is Nothing, FindAncestorFullWeekSched(), Nothing)
        ...
        If host IsNot Nothing Then
            ...
            _dialogExpandedFullWeek = Nothing
            _dialogExpandedFullWeekTemporarily = False
        ElseIf fullWeek IsNot Nothing Then
            _dialogExpandedFullWeek = fullWeek
            _dialogExpandedFullWeekTemporarily = Not fullWeek.IsBodyWorkspaceExpanded
            If _dialogExpandedFullWeekTemporarily Then
                fullWeek.EnsureBodyWorkspaceExpanded()
            End If
            _dialogExpandedHost = Nothing
            _dialogExpandedHostTemporarily = False
        Else
        ...
        Dim overlayHostCtl As Control = Nothing
        If host IsNot Nothing Then overlayHostCtl = host
        If fullWeek IsNot Nothing Then overlayHostCtl = fullWeek
        Dim overlayParent As Control = If(overlayHostCtl, CType(_workArea, Control))
    ...
    Private Sub RestoreDialogOverlayParent()
        ...
        End If
        If _dialogExpandedFullWeekTemporarily AndAlso _dialogExpandedFullWeek IsNot Nothing Then
            _dialogExpandedFullWeek.EnsureBodyWorkspaceCollapsed()
        End If
        _dialogExpandedHost = Nothing
        _dialogExpandedHostTemporarily = False
        _dialogExpandedFullWeek = Nothing
    ...
    Private Function FindAncestorFullWeekSched() As FullWeekSched
        Dim current As Control = Me
        While current IsNot Nothing
            Dim fw = TryCast(current, FullWeekSched)
            If fw IsNot Nothing Then Return fw
            current = current.Parent
        End While
        Return Nothing
    End Function
    ...
    Friend Function BuildHtmlExportContext(weekCaption As String) As WeekSnapshotHtmlContext
        ...
        For i = 0 To _dayHosts.Count - 1
            ...
            For Each c In OrderVisibleWeekApptCardsForLayout(
                sc.Cards.Controls.OfType(Of ApptCard)().Where(Function(card) card.Visible).ToList(),
                If(_request?.Data, Nothing))
...
Friend NotInheritable Class WeekDayCardDrawerHost
    ...
    Private Sub RelayoutDrawerCards()
        ...
        Dim y = 0
        Const gap = 8
        Dim apptCards = _scroll.Controls.OfType(Of ApptCard)().Where(Function(c) c.Visible AndAlso c.BoundAppointment IsNot Nothing).ToList()
        Dim orderedApptCards = ApptWeekCtl.OrderVisibleWeekApptCardsForLayout(apptCards, _currentData)
        For Each c As Control In orderedApptCards.Cast(Of Control)().Concat(
            _scroll.Controls.OfType(Of Control)().Where(Function(ctrl) ctrl.Visible AndAlso Not TypeOf ctrl Is ApptCard).OrderBy(Function(ctrl) _scroll.Controls.GetChildIndex(ctrl)))
            c.Width = w
...
Friend NotInheritable Class WeekDayCardDialogHost
    ...
    Private ReadOnly _btnClose As Label
    Private ReadOnly _scroll As Panel
    Private _dialogData As ApptDataBundle
    ...
    Private Sub LayoutCards()
        ...
        Dim innerW = Math.Max(220, _scroll.ClientSize.Width - _scroll.Padding.Horizontal - 6)
        Dim apptCards = _scroll.Controls.OfType(Of ApptCard)().Where(Function(c) c.Visible AndAlso c.BoundAppointment IsNot Nothing).ToList()
        Dim orderedApptCards = ApptWeekCtl.OrderVisibleWeekApptCardsForLayout(apptCards, _dialogData)
        For Each c As Control In orderedApptCards.Cast(Of Control)().Concat(
            _scroll.Controls.OfType(Of Control)().Where(Function(ctrl) ctrl.Visible AndAlso Not TypeOf ctrl Is ApptCard).OrderBy(Function(ctrl) _scroll.Controls.GetChildIndex(ctrl)))
            c.Left = 0
            c.Top = y
