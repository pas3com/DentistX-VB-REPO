...
#Region "Appointment display order (linked admin doctor, other doctors by name, then by time)"
    ...
    Public Function OrderAppointmentsForDisplay(source As IEnumerable(Of AppointmentC), data As ApptDataBundle, Optional linkedDoctorAtEnd As Boolean = False) As List(Of AppointmentC)
        If data Is Nothing Then
            If source Is Nothing Then Return New List(Of AppointmentC)()
            Return source.OrderBy(Function(a) a.StartDateTime).ThenBy(Function(a) a.EndDateTime).ToList()
        End If
    ...
    ''' Sort units by earliest start in the unit; when that ties, <em>group before solo</em>; then doctor display name, <see cref="AppointmentC.DrID"/>.
    ''' When every doctor has at most one appointment, all units are solo and the list is chronological by start time.
    ''' </summary>
    Public Function OrderAppointmentsForWeekDayGroupsAndSolos(source As IEnumerable(Of AppointmentC), getDoctorName As Func(Of Integer, String)) As List(Of AppointmentC)
        If source Is Nothing Then Return New List(Of AppointmentC)()
    ...
    Public Function OrderAppointmentsForWeekDayGroupsAndSolos(source As IEnumerable(Of AppointmentC), data As ApptDataBundle) As List(Of AppointmentC)
        If data Is Nothing Then
            If source Is Nothing Then Return New List(Of AppointmentC)()
            Return OrderAppointmentsForWeekDayGroupsAndSolos(source, Function(id) "Doctor " & id)
        End If
        Return OrderAppointmentsForWeekDayGroupsAndSolos(source, Function(id) data.ResolveDoctor(id).DrName)
    End Function
