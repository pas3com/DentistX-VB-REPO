...
Public Class ClinicDATA
	...
	Public Function SelectAll() As IEnumerable(Of Clinic)
		Using conn As New SqlConnection(ConnectionString)
			conn.Open()
			Return conn.Query(Of Clinic)("SELECT * FROM Clinic")
		End Using
	End Function


	Public Function Select_Record(ByVal clsClinic As Clinic) As Clinic
		Using conn As New SqlConnection(ConnectionString)
			Dim sql As String = "Select * FROM Clinic WHERE ClinicID = @ClinicID"
			Return conn.QuerySingleOrDefault(Of Clinic)(sql, New With {.ClinicID = clsClinic.ClinicID})
		End Using
	End Function
	...
	Public Function Update(oldClinic As Clinic, newClinic As Clinic) As Boolean
		Using conn As New SqlConnection(ConnectionString)
			Dim parameters = New With {
					.NewClinicNameEn = newClinic.ClinicNameEn,
					.NewClinicNameAr = newClinic.ClinicNameAr,
					.NewDrNameEn = newClinic.DrNameEn,
					.NewDrNameAr = newClinic.DrNameAr,
					.NewSpecialistEn = newClinic.SpecialistEn,
					.NewSpecialistAr = newClinic.SpecialistAr,
					.NewAddressEn = newClinic.AddressEn,
					.NewAddressAr = newClinic.AddressAr,
					.NewPhone = newClinic.Phone,
					.NewMobile = newClinic.Mobile,
					.NewEmail = newClinic.Email,
					.NewClinicLogo = newClinic.ClinicLogo,
					.KeyClinicID = oldClinic.ClinicID
			}
