...
#Region "MouseActivity"
    ...
    Private Sub JawPanel_MouseClick(sender As Object, e As MouseEventArgs) Handles JawPanel.MouseClick
        If e.Button = MouseButtons.Right Then
            ...
            If PatientID <= 0 Then Return
            'ComboFlyoutSearchHelper.ConfigureManualAnchoredFlyoutOnOwner(Flyout1, Me.JawPanel, New Point(JawPanel.Left + e.X, JawPanel.Top + e.Y), True)
            Flyout1.ShowPopup()
...
#End Region
    ...
    Private Sub CommonMouseClickHandler(sender As Object, e As MouseEventArgs)
        ...
        If e.Button = MouseButtons.Right Then
            ...
            'ComboFlyoutSearchHelper.ConfigureLegacyToothFlyout(Flyout1, Me.JawPanel, Me.JawPanel, svg, StringComparison.Ordinal)


            Dim loc As Point = svg.Location
            Flyout1.OwnerControl = sender
            Flyout1.Options.AnchorType = DevExpress.Utils.Win.PopupToolWindowAnchor.Manual
            If svg.Name.StartsWith("Ld") Then
                Flyout1.Options.Location = New Point(loc.X - Flyout1.Width, loc.Y + svg.Height - Flyout1.Height)
                Flyout1.ShowBeakForm(New Point(loc.X - Flyout1.Width, loc.Y + svg.Height - Flyout1.Height), True)
            ElseIf svg.Name.StartsWith("Rd") Then
                Flyout1.Options.Location = New Point(loc.X + svg.Width, loc.Y + svg.Height - Flyout1.Height)
                Flyout1.ShowBeakForm(New Point(loc.X + svg.Width, loc.Y + svg.Height - Flyout1.Height), True)
            ElseIf svg.Name.StartsWith("Lu") Then
                Flyout1.Options.Location = New Point(loc.X - Flyout1.Width, loc.Y)
                Flyout1.ShowBeakForm(New Point(loc.X - Flyout1.Width, loc.Y), True)
            ElseIf svg.Name.StartsWith("Ru") Then
                Flyout1.Options.Location = New Point(loc.X + svg.Width, loc.Y)
                Flyout1.ShowBeakForm(New Point(loc.X + svg.Width, loc.Y), True)
            End If
