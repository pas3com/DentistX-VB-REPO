...
#Region "The Class"
    ...
    Public Sub New()
        ...
        'LogToFile("AdultLowerJaw_New Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
        'LoadPatientTreats(PatientID)
        LogTime("AdultLowerJaw_New", Me.Name, sw)
    End Sub
    ...
    Public Sub New(ByVal PatientID As Integer) ', hideDiv As Boolean)
        ...
        sw.Stop()
        LogToFile("AdultLowerJaw_NewByVal PatientID Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
    End Sub
    ...
    WHERE  [TrtGroup] Is Not NULL   AND [TrtGroup] <> 'DIAG' 
        ...
        (
            ...
            For Each row As DataRow In dataTable.Rows
                Dim nodeInfo As New TrtSourceHelper.TrtNodeInfo With {
...
#Region "ENTER KEY AND DOUBLE CLICK AddedTrts AND TreeView Events"
    ...
    Private Sub TreeView_NodeMouseDoubleClick(sender As Object, e As TreeNodeMouseClickEventArgs) Handles TrtsTreeView.NodeMouseDoubleClick 
        ...
        If (svgSelectedList Is Nothing OrElse svgSelectedList.Count = 0) AndAlso DragSource Is Nothing Then Exit Sub

        ' Collect target SVGs (single or multiple selection)
        Dim targetSVGs As List(Of SvgImageBox) = If(svgSelectedList IsNot Nothing AndAlso svgSelectedList.Count > 0, svgSelectedList, New List(Of SvgImageBox) From {DragSource})
...
#Region "Loading"
    ...
    Private Sub AdultLowerJaw_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim sw = StartTimer()
        'Dim sw As New Stopwatch
        'sw.Start()
        ...
        'sw.Stop()
        'LogToFile("AdultLowerJaw_Load Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
        LogTime(NameOf(AdultLowerJaw_Load), Me.Name, sw)
    End Sub
    ...
    Private Sub AdultLowerJaw_Resize(sender As Object, e As EventArgs) Handles Me.Resize
        If controlBoundsCache.IsEmpty Then Return
        ResizeControlsProportionally()
        ALIGNS()
    End Sub
...
#Region "FlyOutEvents"
    ...
    Private NotInheritable Class FlyMenuOutsideClickFilter
        ...
        Private ReadOnly _jaw As AdultLowerJaw

        Public Sub New(jaw As AdultLowerJaw)
            _jaw = jaw
        End Sub
...
#Region "Aligning"
    Public Sub AlignTops()
        ...
        Dim rdINTop As Integer = RdIN1.Top

        For Each ctl As Control In JawPanel.Controls
            If ctl.Name.Contains("Top") Then
                If ctl.Name.StartsWith("Rd") OrElse ctl.Name.StartsWith("Ld") Then
                    ctl.Top = rdTop
                End If
            ElseIf ctl.Name.Contains("IN") Then
                If ctl.Name.StartsWith("Rd") OrElse ctl.Name.StartsWith("Ld") Then
                    ctl.Top = rdINTop
                End If
    ...
    Private Sub AdultLowerJaw_Disposed(sender As Object, e As EventArgs) Handles Me.Disposed
