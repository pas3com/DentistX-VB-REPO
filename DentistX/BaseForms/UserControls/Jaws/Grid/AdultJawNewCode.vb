...
Public Class AdultJawNewCode
    ...
    'Private Sub AdultJawNewCode_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "The Class"
    ...
    Public Sub New()
        ...
        'LogToFile("AdultJawNewCode_New Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
        LoadPatientTreats(PatientID)
        LogTime("AdultJawNewCode_New", Me.Name, sw)
    End Sub
    ...
    Public Sub New(ByVal PatientID As Integer) ', hideDiv As Boolean)
        ...
        sw.Stop()
        LogToFile("AdultJawNewCode_NewByVal PatientID Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
    End Sub
    ...
    Private Sub SetSvgImages1()
        Dim resourceMap = Helpers.AdultResourceMapping
        Debug.WriteLine($"Setting SVG images. Resource map count: {resourceMap.Count}")
        ...
        For Each ct As Control In Me.JawPanel.Controls
            If TypeOf ct Is SvgImageBox Then
                ...
                Debug.WriteLine($"Control: {sv.Name} -> BaseKey: {baseKey}")
                ...
                If resourceMap.ContainsKey(baseKey) Then
                    ...
                    If sv.Name.Contains("Out") Then
                        ...
                        Debug.WriteLine($"Set Out image for {baseKey}")
                    ElseIf sv.Name.Contains("Top") Then
                        sv.SvgImage = resources.SvgTopResource
                        Debug.WriteLine($"Set Top image for {baseKey}")
                    ElseIf sv.Name.Contains("IN") Then
                        sv.SvgImage = resources.SvgInResource
                        Debug.WriteLine($"Set In image for {baseKey}")
                    Else
                        sv.SvgImage = Nothing
                    End If
                Else
                    Debug.WriteLine($"BaseKey not found in mapping: {baseKey}")
                End If
            End If
        Next
    End Sub
...
#Region "Definitions"
    ...
    ''' <summary>Global current patient (FormManager) with fallback to local clsPatient. Use for all patient-related operations.</summary>
    Public ReadOnly Property CurrentPatient As Patient
        ...
        End Get
    End Property
    ''' <summary>Patient ID from global CurrentPatient or local pid.</summary>
    Public ReadOnly Property PatientID As Integer
        Get
            Dim p = CurrentPatient
            If p IsNot Nothing Then Return p.PatientID
            Return pid
        End Get
    End Property
    ''' <summary>Patient name from global CurrentPatient or local clsPatient.</summary>
...
#Region "LoadingTreatsSubs"
    ...
    Public Sub LoadTeethTreatsUsingTreatCode(patientTreats As IEnumerable(Of Patient_ToothTrt))
        ...
        Try
            ...
            For Each ct As Control In JawPanel.Controls.OfType(Of SvgImageBox)()
                ...
                'TreatHelper.ProcessToothTreatments1(svg, svgExternalList, svgDiagList, patientTreats)
                svg.EndUpdate()
            Next
    ...
    Public Sub LoadTeethTreats(patientTreats As IEnumerable(Of Patient_ToothTrt))
        ...
        JawPanel.SuspendLayout()
        Me.Visible = False
        Dim treats = If(PatientTreats, Enumerable.Empty(Of Patient_ToothTrt)())
...
#Region "Single Treat"
    ...
    Public Sub LoadPatientSnglToothTreat(patientTreats As IEnumerable(Of Patient_ToothTrt))
        ...
        'TreatHelper.ProcessToothTreatments1(zSvg, svgExternalList, svgDiagList, patientTreats)
...
#Region "Selection Methods"
    ...
    Private Sub SelectSingleViewOfTooth(svg As SvgImageBox, toothNum As Byte)
    ...
    Private Sub DeselectAllViewsOfTooth(toothNum As Byte)
    ...
    Private Sub RemoveToothFromAddList(toothNum As Byte)
        ...
        For Each item As String In addList.Items
            If item.Contains(toothNum.ToString()) Then
    ...
    Private Sub UnSelect()
        ...
        For Each ct As Control In Me.JawPanel.Controls
            If TypeOf ct Is SvgImageBox Then
                Dim sv As SvgImageBox = CType(ct, SvgImageBox)
                'Dim col As SvgImageItemCollection = sv.RootItems
                'If col.Find(Function(c) c.Id = "CON") IsNot Nothing Then
                '    col.Find(Function(c) c.Id = "CON").Visible = True
                '    col.Find(Function(c) c.Id = "CON").Appearance.Normal.BorderThickness = 0
                'End If
                sv.Text = ""
                If Not svgSelectedList.Contains(sv) OrElse Not svgExternalList.Contains(sv) Then
                    sv.BackColor = Color.Transparent
                    'Else
                    '    MsgBox(svgExternalList.Find(Function(s) s.Name = sv.Name)).ToString()
                End If
            End If
        ...
        End If
        sw.Stop()
        LogToFile("UnSelect Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
    End Sub
...
#End Region
    ...
    Private Sub CommonMouseClickHandler(sender As Object, e As MouseEventArgs)
        ...
        If e.Button = MouseButtons.Right Then
            ...
            ' Use Application.DoEvents to keep UI responsive during processing
            ...
            Try
                If Not IsMobile Then
                    ...
                    If targetCount <= 1 Then
                        SetTreeTreatsSingleTooth(toothID, toothNum) ' Use optimized version
                    Else
                        SetTrtsTreeMultiTeethNewCombined() ' Use optimized version
                    End If
                Else
                    SetAddedMobsListDataSource(PatientID, toothID, GetToothFullName(toothName, TreatsUserControl.AlternateQuadrantLabelsEnabled))
                    If targetCount <= 1 Then
                        SetMobTreeSnglTooth(toothID, toothNum) ' Use optimized version
                    Else
                        SetMobTreeMultiTeeth() ' Use optimized version
                    End If
                End If
            Finally
        ...
        ElseIf e.Button = MouseButtons.Left Then
            ...
            If Control.ModifierKeys = Keys.Control Then
                ...
                Else
                    ...
                    SelectSingleViewOfTooth(svg, ToothNum)
                    ...
                    For Each sv In selectedSvgs
                        If sv.Name.Contains("Top") Then
                            Dim outView As SvgImageBox = FindOutView(CByte(sv.Tag))
                            If outView IsNot Nothing Then
                                SelectSvg(outView)
                            End If
                        Else
            ...
            Else
                ...
                End If
                ClearSvgSelection()
                'GroupControl2.Visible = selectedTeethList.Count > 0
                slctdSVG = CType(sender, SvgImageBox)
                UnSelect()
...
#Region "New TreeViewSource"
    ...
    ' Value-type for node Inf (safe to store as Tag)
    Private Structure TrtNodeInfo1
    ...
    Public Class TrtNodeInfo
        ...
        Public Property TrtDetails As String
        Public Property TrtLVL As Integer
        Public Property ToothID As String
        Public Property OldTrt As String
        Public Property TrtGroup As String
        Public Property ParentGroup As String
        Public Property KidTrt As Integer
    End Class
    ...
    Private Sub SetTrtsTreeMultiTeeth()
        ...
        Try
            ...
            If TrtSourceHelper.PatientHasDentureOnAnyTooth(PatientID, selectedTeethList.Select(Function(t) CInt(t))) Then
                AllTrtNodes.Clear()
                Dim empty As New List(Of TrtSourceHelper.TrtNodeInfo)()
                TrtSourceHelper.BuildTreeFromList(TrtsTreeView, empty, Nothing, "No treatments available: a selected tooth has a denture.")
            ...
            For Each row As DataRow In dataTable.Rows
                Dim nodeInfo As New TrtNodeInfo With {
                .TrtID = row("TrtID").ToString(),
                ...
                .ShapeID = If(IsDBNull(row("ShapeID")), Nothing, row("ShapeID").ToString()),
                .TrtBrdrThick = row.Field(Of Byte?)("TrtBrdrThick").GetValueOrDefault(0)
            }
                AllTrtNodes.Add(nodeInfo)
            Next

            BuildTreeFromList(AllTrtNodes, existingTrts)
    ...
    Private Sub SetTrtsTreeMultiTeethNew()
        Try
            TrtsTreeView.Nodes.Clear()
            If TrtSourceHelper.PatientHasDentureOnAnyTooth(PatientID, selectedTeethList.Select(Function(t) CInt(t))) Then
                AllTrtNodes.Clear()
            ...
            ' ------------------------
            ' 0. Determine teeth/jaw stats (YOUR ORIGINAL CODE)
            ' ------------------------
            Dim hasUpperTeeth As Boolean = selectedTeethList.Any(Function(t) t >= 11 AndAlso t <= 28)
            Dim hasLowerTeeth As Boolean = selectedTeethList.Any(Function(t) t >= 31 AndAlso t <= 48)
            Dim isMixedSelection As Boolean = hasUpperTeeth AndAlso hasLowerTeeth
            ...
            ' ------------------------
            ' 1. MODIFIED QUERY - Use STRING_SPLIT for TrtGroup
            ' ------------------------
            Dim query As String = "
;WITH TreatmentGroups AS (
    SELECT 
        ...
        t.[ToothID], 
        t.[OldTrt],  
        LTRIM(RTRIM(sg.value)) AS TrtGroup,  -- Split and clean groups
        t.[ParentGroup], 
        t.[KidTrt],
        s.[ShapeName]
    FROM [TblTRTS] t
    CROSS APPLY STRING_SPLIT(t.TrtGroup, ',') sg
    LEFT JOIN [Shapes] s ON t.[ShapeID] = s.[ShapeID]  
    WHERE t.TrtGroup IS NOT NULL 
      AND t.TrtGroup <> '' 
)
SELECT * FROM TreatmentGroups
WHERE 1=1
  AND (
      -- Always show ALL treatments
      [ToothID] = 'ALL' 
      ...
      OR (
          -- Check if treatment matches any selected tooth
          (
              EXISTS(SELECT value FROM STRING_SPLIT(@ToothID, ',') WHERE ',' + [ToothID] + ',' LIKE '%,' + value + ',%')
              OR EXISTS(SELECT value FROM STRING_SPLIT([ToothID], ',') WHERE ',' + @ToothID + ',' LIKE '%,' + value + ',%')
          )
          AND (
              -- Case 1: When mixing both 1-3 And 4-8 - only show Class 1 And 5
              (
            ...
            ' ------------------------
            ' 2. EXECUTE QUERY AND GET EXISTING TREATMENTS (YOUR ORIGINAL CODE)
            ' ------------------------
            Dim connectionString As String = DentistXDATA.GetConnection.ConnectionString
            Dim connection As New SqlConnection(connectionString)

            ' YOUR ORIGINAL existingTrts LOGIC
            ...
            ' Store treatments by group for tree building
            Dim treatmentsByGroup As New Dictionary(Of String, List(Of TrtNodeInfo))()
            ...
            For Each row As DataRow In dataTable.Rows
                ...
                .TrtValue = row.Field(Of Decimal?)("TrtValue").GetValueOrDefault(0D),
                .ParentGroup = row("ParentGroup").ToString(),
                .TrtGroup = row("TrtGroup").ToString(), ' This is now individual group
                .TrtColor = row.Field(Of String)("TrtColor"),
                .TrtLoc = row.Field(Of String)("TrtLoc"),
                .TrtBrdrClr = row.Field(Of String)("TrtBrdrClr"),
                .ShapeName = row.Field(Of String)("ShapeName"),
                ...
                ' Group treatments by their TrtGroup
                If Not treatmentsByGroup.ContainsKey(nodeInfo.TrtGroup) Then
                    treatmentsByGroup(nodeInfo.TrtGroup) = New List(Of TrtNodeInfo)()
                End If
                treatmentsByGroup(nodeInfo.TrtGroup).Add(nodeInfo)

                AllTrtNodes.Add(nodeInfo)
            Next

            ' ------------------------
            ' 3. BUILD TREE WITH MULTI-GROUP SUPPORT
            ' ------------------------
            BuildTreeWithMultipleGroups(treatmentsByGroup, existingTrts)

            ' ------------------------
            ' 4. YOUR ORIGINAL FINALIZATION CODE
            ' ------------------------
            SaveFullTreeSnapshot()
        ...
        End Try
    End Sub
    Private Sub BuildTreeWithMultipleGroups(treatmentsByGroup As Dictionary(Of String, List(Of TrtNodeInfo)), existingTrts As HashSet(Of Integer))
        TrtsTreeView.Nodes.Clear()

        ' Group by ParentGroup first
        Dim parentGroups As New Dictionary(Of String, Dictionary(Of String, List(Of TrtNodeInfo)))()

        ' Organize treatments by ParentGroup and then by TrtGroup
        For Each groupEntry In treatmentsByGroup
            Dim groupName As String = groupEntry.Key
            Dim treatmentsList As List(Of TrtNodeInfo) = groupEntry.Value

            For Each treatment In treatmentsList
                Dim parentGroup = treatment.ParentGroup

                If Not parentGroups.ContainsKey(parentGroup) Then
                    parentGroups(parentGroup) = New Dictionary(Of String, List(Of TrtNodeInfo))()
                End If

                If Not parentGroups(parentGroup).ContainsKey(groupName) Then
                    parentGroups(parentGroup)(groupName) = New List(Of TrtNodeInfo)()
                End If

                ' Add treatment to the appropriate group
                parentGroups(parentGroup)(groupName).Add(treatment)
            Next
        Next

        ' Build tree structure
        For Each parentGroupEntry In parentGroups
            Dim parentGroupName As String = parentGroupEntry.Key
            Dim childGroups As Dictionary(Of String, List(Of TrtNodeInfo)) = parentGroupEntry.Value

            Dim parentNode As New TreeNode(parentGroupName)
            parentNode.Tag = "PARENT_GROUP"

            For Each childGroupEntry In childGroups
                Dim groupName As String = childGroupEntry.Key
                Dim groupTreatments As List(Of TrtNodeInfo) = childGroupEntry.Value

                Dim groupNode As New TreeNode(groupName)
                groupNode.Tag = "TREATMENT_GROUP"

                For Each treatment In groupTreatments
                    Dim treatmentNode As New TreeNode($"{treatment.Trt} - {treatment.Trt}")
                    treatmentNode.Tag = treatment
                    treatmentNode.ForeColor = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)), Color.Gray, Color.Black)

                    ' Store all treatment Inf in node
                    treatmentNode.Name = treatment.TrtID.ToString()
            ...
            TrtsTreeView.Nodes.Add(parentNode)
        Next
    End Sub

    Private Sub SetTrtsTreeMultiTeethNewCombined()
        Try
            TrtsTreeView.Nodes.Clear()
            If TrtSourceHelper.PatientHasDentureOnAnyTooth(PatientID, selectedTeethList.Select(Function(t) CInt(t))) Then
                AllTrtNodes.Clear()
            ...
            End If
            ' ------------------------
            ' 0. Determine teeth/jaw stats (keep existing code)
            ' ------------------------
            Dim hasUpperTeeth As Boolean = selectedTeethList.Any(Function(t) t >= 11 AndAlso t <= 28)
            Dim hasLowerTeeth As Boolean = selectedTeethList.Any(Function(t) t >= 31 AndAlso t <= 48)
            Dim isMixedSelection As Boolean = hasUpperTeeth AndAlso hasLowerTeeth
...
;WITH TreatmentGroups AS (
    SELECT 
        ...
        t.[ToothID], 
        t.[OldTrt],  
        LTRIM(RTRIM(sg.value)) AS TrtGroup,  -- Split and clean TrtGroup
        LTRIM(RTRIM(pg.value)) AS ParentGroup,  -- Split and clean ParentGroup
        t.[KidTrt],
        s.[ShapeName]
    FROM [TblTRTS] t
    CROSS APPLY STRING_SPLIT(t.TrtGroup, ',') sg
    CROSS APPLY STRING_SPLIT(t.ParentGroup, ',') pg  -- Split ParentGroup too
    LEFT JOIN [Shapes] s ON t.[ShapeID] = s.[ShapeID]  
    WHERE t.TrtGroup IS NOT NULL 
      AND t.TrtGroup <> '' 
      AND t.ParentGroup IS NOT NULL
      AND t.ParentGroup <> ''
)
SELECT * FROM TreatmentGroups
WHERE 1=1 
  AND (
      ...
      OR (
          ...
          AND (
            ...
            '===========================
            Select Case True
    ' Case 0000: No filters - Show everything
                Case Not norTrt AndAlso Not exTrt AndAlso Not impTrt AndAlso Not brgTrt
                    query += " AND 1=1"
                    ' All other cases: Show treatments that match ANY selected filter
                Case Else
                    Dim conditions As New List(Of String)
                    If norTrt Then
                        conditions.Add("([TrtGroup] NOT IN ('IMPLANT', 'CROWNS ON IMPLANT', 'IMPLANT COMPONENT', 'BRIDGE'))")
                    End If
                    If exTrt Then
                        'conditions.Add("([ParentGroup] IN ('IMPLANT', 'BRIDGE', 'CROWNS ON IMPLANT'))")
                        conditions.Add("([ParentGroup] IN ('IMPLANT', 'CROWNS ON IMPLANT', 'IMPLANT COMPONENT') OR [TrtGroup] = 'BRIDGE') AND [Trt] <> 'IMPLANT + EXTRACTION'")
                    End If
                    If impTrt Then
                        conditions.Add("([Trt] = 'EXTRACTION' OR [TrtGroup] = 'BRIDGE' OR [ParentGroup] IN ('IMPLANT', 'CROWNS ON IMPLANT', 'IMPLANT COMPONENT'))")
                    End If
                    If brgTrt Then
                        conditions.Add("([Trt] = 'EXTRACTION' OR [TrtLoc] = 'ROOT' OR [TrtGroup] = 'BRIDGE')")
                    End If
            ...
            query += " ORDER BY [ParentGroup], [TrtGroup], [Trt]"
            ' ------------------------
            ' 2. MODIFIED: Execute query and build tree with multi-group support
            ' ------------------------
            Dim connectionString As String = DentistXDATA.GetConnection.ConnectionString
            Dim connection As New SqlConnection(connectionString)
            Dim toothNums As New List(Of Byte)
            ...
            ' ------------------------
            ' 3. MODIFIED: Build tree with multi-group support
            ' ------------------------
            BuildTreeWithMultipleGroupsMulti(treatmentsByGroup, existingTrts)
            SaveFullTreeSnapshot()
            For Each n As TreeNode In TrtsTreeView.Nodes
                If n.Tag?.ToString() = "PARENT_GROUP" Then n.Collapse()
            Next
        ...
        End Try
    End Sub
    Private Sub BuildTreeWithMultipleGroupsMulti(treatmentsByGroup As Dictionary(Of String, List(Of TrtNodeInfo)), existingTrts As HashSet(Of Integer))
        TrtsTreeView.Nodes.Clear()
        ' Create dictionaries to organize nodes
        Dim parentGroupNodes As New Dictionary(Of String, TreeNode)()
        Dim trtGroupNodes As New Dictionary(Of String, TreeNode)()
        ' First pass: Create all parent group nodes
        For Each groupEntry In treatmentsByGroup
            Dim treatmentsList As List(Of TrtNodeInfo) = groupEntry.Value
            For Each treatment In treatmentsList
                Dim parentGroupName = treatment.ParentGroup
                ' Create parent group node if it doesn't exist
                If Not parentGroupNodes.ContainsKey(parentGroupName) Then
                    Dim parentNode As New TreeNode(parentGroupName)
                    parentNode.Tag = "PARENT_GROUP"
                    parentNode.ForeColor = Color.DarkBlue
                    parentNode.NodeFont = New Font(TrtsTreeView.Font, FontStyle.Bold)
                    parentGroupNodes(parentGroupName) = parentNode
                    TrtsTreeView.Nodes.Add(parentNode)
                End If
            Next
        Next
        ' Second pass: Create treatment group nodes under their parent groups
        For Each groupEntry In treatmentsByGroup
            Dim groupName As String = groupEntry.Key
            Dim treatmentsList As List(Of TrtNodeInfo) = groupEntry.Value
            For Each treatment In treatmentsList
                Dim parentGroupName = treatment.ParentGroup
                Dim groupKey = $"{parentGroupName}|{groupName}"
                ' Create treatment group node if it doesn't exist
                If parentGroupNodes.ContainsKey(parentGroupName) AndAlso Not trtGroupNodes.ContainsKey(groupKey) Then
                    Dim groupNode As New TreeNode(groupName)
                    groupNode.Tag = "TREATMENT_GROUP"
                    groupNode.ForeColor = Color.DarkGreen
                    trtGroupNodes(groupKey) = groupNode
                    parentGroupNodes(parentGroupName).Nodes.Add(groupNode)
                End If
            Next
        Next
        ' Third pass: Add treatments to their groups
        For Each groupEntry In treatmentsByGroup
            Dim groupName As String = groupEntry.Key
            Dim treatmentsList As List(Of TrtNodeInfo) = groupEntry.Value
            For Each treatment In treatmentsList
                Dim parentGroupName = treatment.ParentGroup
                Dim groupKey = $"{parentGroupName}|{groupName}"
                If trtGroupNodes.ContainsKey(groupKey) Then
                    Dim trtstr As String = ""
                    If parentGroupName.Contains("DIAGNOSIS") Then
                        trtstr = $"{treatment.Trt} - {parentGroupName}"
                    Else
                        trtstr = $"{treatment.Trt}"
                    End If
                    Dim treatmentNode As New TreeNode(trtstr) '$"{treatment.Trt} - {parentGroupName}")
                    'treatmentNode = $"{treatment.Trt} - {parentGroupName}"
                    treatmentNode.Tag = treatment
                    treatmentNode.ForeColor = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)), Color.Red, Color.Black)
                    ' Simple font style assignment
                    treatmentNode.NodeFont = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)),
                               New Font(TrtsTreeView.Font, FontStyle.Strikeout), TrtsTreeView.Font)
                    treatmentNode.Name = treatment.TrtID.ToString()
                    ' Optional: Add treatment details as tooltip
                    treatmentNode.ToolTipText = $"Value: {treatment.TrtValue} | Location: {treatment.TrtLoc}"
                    trtGroupNodes(groupKey).Nodes.Add(treatmentNode)
                End If
            Next
        Next
        ' Auto-expand parent groups that have only one child for better visibility
        For Each parentNode In parentGroupNodes.Values
    ...
    Private Sub BuildTreeWithCombinedParentGroups(treatmentsByGroup As Dictionary(Of String, List(Of TrtNodeInfo)), existingTrts As HashSet(Of Integer))
        TrtsTreeView.Nodes.Clear()

        Dim allNodes As New Dictionary(Of String, TreeNode)()
        ...
        For Each groupEntry In treatmentsByGroup
            ...
            For Each treatment In treatmentsList
                ...
                ' Create combined parent group node
                Dim parentKey = $"PARENT_{parentGroupName}"
                If Not allNodes.ContainsKey(parentKey) Then
                    Dim parentNode As New TreeNode(parentGroupName)
                    parentNode.Tag = "PARENT_GROUP"
                    parentNode.ForeColor = Color.DarkBlue
                    parentNode.NodeFont = New Font(TrtsTreeView.Font, FontStyle.Bold)
                    allNodes(parentKey) = parentNode
                    TrtsTreeView.Nodes.Add(parentNode)
                End If

                ' Create treatment group node
                Dim groupKey = $"{parentGroupName}|{groupName}"
                If Not allNodes.ContainsKey(groupKey) Then
                    Dim groupNode As New TreeNode(groupName)
                    groupNode.Tag = "TREATMENT_GROUP"
                    groupNode.ForeColor = Color.DarkGreen
                    allNodes(groupKey) = groupNode
                    allNodes(parentKey).Nodes.Add(groupNode)
                End If

                ' Add treatment
                Dim treatmentKey = $"{groupKey}|{treatment.TrtID}"
                If Not allNodes.ContainsKey(treatmentKey) Then
                    Dim treatmentNode As New TreeNode($"{treatment.Trt} - {treatment.Trt}")
                    treatmentNode.Tag = treatment
                    treatmentNode.ForeColor = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)), Color.Gray, Color.Black)
                    treatmentNode.Name = treatment.TrtID.ToString()
                    allNodes(treatmentKey) = treatmentNode
                    allNodes(groupKey).Nodes.Add(treatmentNode)
                End If
            Next
        Next
    End Sub

    Private Sub SetTreeTreatsSingleToothOriginal(toothID As String, toothNum As Byte)
        ...
        Using connection As New SqlConnection(connectionString)
            ...
            ' ----------------------------------------------------------
            Dim existingTrts As New HashSet(Of Integer)
            Using checkCmd As New SqlCommand("SELECT TrtID FROM dbo.Patient_ToothTrt INNER JOIN
                         dbo.TblTRTS ON dbo.Patient_ToothTrt.Treat = dbo.TblTRTS.Trt WHERE PatientID = @PatientID AND ToothNum = @ToothNum", connection)
                checkCmd.Parameters.AddWithValue("@PatientID", PatientID)
            ...
            ' ----------------------------------------------------------SELECT [TblTRTS].[TrtID], [TblTRTS].[Trt], [TblTRTS].[TrtColor], [TblTRTS].[TrtBrdrClr],
            Dim query As String = " SELECT [TblTRTS].[TrtID], [TblTRTS].[Trt], [TblTRTS].[TrtValue], [TblTRTS].[TrtLoc], [TblTRTS].[TrtColor], [TblTRTS].[TrtBrdrClr], 
            [TblTRTS].[TrtBrdrThick], [TblTRTS].[ShapeID], [Shapes].[ShapeName], [TblTRTS].[TrtDetails],
            [TblTRTS].[TrtLVL], [TblTRTS].[ToothID], [TblTRTS].[OldTrt],
            [TblTRTS].[TrtGroup], [TblTRTS].[ParentGroup], [TblTRTS].[KidTrt] 
            From [TblTRTS] LEFT Join [Shapes] On [TblTRTS].[ShapeID] = [Shapes].[ShapeID] 
            ...
            If currentToothLevel = -1 Then
                query += " AND (([TrtGroup] <> 'IMPLANT' AND [TrtGroup] <> 'CROWNS ON IMPLANT' AND [TrtGroup] <> 'IMPLANT COMPONENT') OR [TrtGroup] = 'DENTURES') "
            ElseIf currentToothLevel = 4 Then
                query += " AND (([TrtLVL] > 4 AND [TrtGroup] <> 'CROWNS ON TOOTH' " &
                     ...
                     "AND [Trt] <> 'ABUTMENT' AND [Trt] <> 'HEALING CAP') OR [TrtGroup] = 'DENTURES')"
            ElseIf currentToothLevel = 5 Then
                query += " AND (((( [TrtGroup] = 'EXTRACTION' OR [TrtGroup] = 'BRIDGE') OR ([TrtLVL] > @CurrentToothLevel))  AND [TrtGroup] <> 'CROWNS ON TOOTH' " &
                     "AND [TrtGroup] <> 'INDIRECT VENEERS' AND [TrtGroup] <> 'DIRECT VENEERS' AND [TrtGroup] <> 'CROWNS ON IMPLANT') OR [TrtGroup] = 'DENTURES') "
            ElseIf currentToothLevel = 6 Then
                query += " AND (((( [TrtGroup] = 'EXTRACTION' OR [TrtGroup] = 'BRIDGE') OR ([TrtLVL] >= @CurrentToothLevel))  AND [TrtGroup]  <> 'CROWNS ON TOOTH' " &
                     "AND [TrtGroup] <> 'INDIRECT VENEERS' AND [TrtGroup] <> 'DIRECT VENEERS'  AND [TrtGroup] <> 'IMPLANT') OR [TrtGroup] = 'DENTURES')"
            ElseIf currentToothLevel = 7 Then
                query += " AND (((( [TrtGroup] = 'EXTRACTION' OR [TrtGroup] = 'BRIDGE') OR ([TrtLVL] = @CurrentToothLevel)) AND [TrtGroup]  <> 'CROWNS ON TOOTH' " &
                     "AND [TrtGroup] <> 'IMPLANT' AND [TrtGroup] <> 'INDIRECT VENEERS' AND [TrtGroup] <> 'DIRECT VENEERS' AND [TrtGroup] <> 'CROWNS ON IMPLANT') OR [TrtGroup] = 'DENTURES')"
            ElseIf currentToothLevel = 8 Then
            ...
            For Each row As DataRow In dataTable.Rows
                ...
                .Tag = New With {
                    ...
                    Key .TrtLoc = row.Field(Of String)("TrtLoc"),
                    Key .TrtValue = row.Field(Of Decimal?)("TrtValue").GetValueOrDefault(0D),
                    Key .BorderColor = row.Field(Of String)("TrtBrdrClr"),
                    Key .BorderThick = row.Field(Of Byte?)("TrtBrdrThick").GetValueOrDefault(0),
                    Key .treatText = row.Field(Of String)("TrtColor"),
                    Key .Trt = row.Field(Of String)("Trt"),
                    Key .TrtGroup = row.Field(Of String)("TrtGroup"),
                    Key .ParentGroup = row.Field(Of String)("ParentGroup")
                },
            ...
            ' Show Inf if nothing is available
            If TrtsTreeView.Nodes.Count = 0 Then
    ...
    Private Sub SetTreeTreatsSingleTooth(toothID As String, toothNum As Byte)
        ...
        ' ----------------------------------------------------------
        ' STEP 1: Get current treatment level and existing treatments
        ' ----------------------------------------------------------
        Dim currentToothLevel As Integer = clsToothTrtData.GetTreatLVL(PatientID, toothNum)
        ...
        ' ----------------------------------------------------------
        ' STEP 2: Load and filter treatments - WITH PARENTGROUP SPLITTING
        ' ----------------------------------------------------------
        Dim allTrts As List(Of TblTRTS) = TblTRTSDATA.SelectAll()
        Dim allShapes As List(Of Shapes) = ShapesDATA.SelectAll()

        ' Create a list to hold split treatments
        Dim splitTreatments As New List(Of TrtNodeInfo)()

        For Each t In allTrts
            ' Skip if conditions don't match
            If t.TrtGroup Is Nothing Then Continue For
            If t.ToothID <> "ALL" AndAlso toothID <> "ALL" AndAlso
           Not ("," & t.ToothID & ",").Contains("," & toothID & ",") AndAlso
           Not ("," & toothID & ",").Contains("," & t.ToothID & ",") Then Continue For

            ' Kid/adult condition
            If isKid AndAlso (t.KidTrt <> 1 AndAlso t.KidTrt <> 2) Then Continue For
            If Not isKid AndAlso (t.KidTrt <> 0 AndAlso t.KidTrt <> 2) Then Continue For

            ' Level-specific conditions (same as before)
            ' Level-specific conditions - REFINED VERSION
            Select Case currentToothLevel
                Case -1, 0, 1, 2, 3
                    If t.TrtGroup <> "DENTURES" Then
                        ' Normal treatments - exclude most implants but allow IMPLANT + EXTRACTION from both SURGICAL and IMMEDIATE
                        If t.ParentGroup IsNot Nothing AndAlso t.ParentGroup.Contains("IMPLANT") Then
                            ' Allow IMPLANT + EXTRACTION treatments from both SURGICAL and IMMEDIATE groups
                            If t.Trt <> "IMPLANT + EXTRACTION" Then
                                Continue For
                            End If
                        ElseIf t.TrtGroup = "BRIDGE" Then
                            Continue For
                        End If
                    End If
                ...
                Case 4
                    If t.TrtGroup <> "DENTURES" Then
                        ' Extraction level - show higher level treatments but exclude some
                        If t.TrtLVL <= 4 Then Continue For
                        If t.ParentGroup IsNot Nothing AndAlso t.ParentGroup.Contains("IMPLANT") Then
                            ' Allow IMPLANT + EXTRACTION treatments from both SURGICAL and IMMEDIATE groups
                            If t.Trt.Contains("+") Then
                                Continue For
                            End If


                        ElseIf t.TrtGroup = "CROWNS ON TOOTH" OrElse
                                   t.TrtGroup = "INDIRECT VENEERS" OrElse
                                   t.TrtGroup = "DIRECT VENEERS" OrElse
                                   t.Trt = "ABUTMENT" OrElse t.Trt = "HEALING CAP" Then
                            Continue For
                        End If
                    End If
                ...
                Case 5, 6, 7, 8
                    If t.TrtGroup <> "DENTURES" Then
                        ' Implant levels - show extractions, bridges, and higher level treatments
                        If t.TrtGroup <> "EXTRACTION" AndAlso
                                   t.TrtGroup <> "BRIDGE" AndAlso
                                   t.TrtLVL <= currentToothLevel Then Continue For

                        ' Only exclude crown-related treatments, NOT implants
                        If t.TrtGroup = "CROWNS ON TOOTH" OrElse
                                   t.TrtGroup = "INDIRECT VENEERS" OrElse
                                   t.TrtGroup = "DIRECT VENEERS" Then Continue For
                    End If
            End Select
            'Select Case currentToothLevel
            '    Case -1, 0, 1, 2, 3
            '        If t.ParentGroup IsNot Nothing AndAlso
            '       (t.ParentGroup = "IMPLANT" OrElse t.TrtGroup = "BRIDGE") Then Continue For
            '    Case 4
            '        If t.TrtLVL <= 4 OrElse
            '       t.TrtGroup = "CROWNS ON TOOTH" OrElse
            '       t.TrtGroup = "INDIRECT VENEERS" OrElse
            '       t.TrtGroup = "DIRECT VENEERS" OrElse
            '       (t.ParentGroup IsNot Nothing AndAlso t.ParentGroup = "CROWNS ON IMPLANT") OrElse
            '       t.Trt = "ABUTMENT" OrElse t.Trt = "HEALING CAP" Then Continue For
            '    Case 5
            '        If (t.TrtGroup <> "EXTRACTION" AndAlso
            '        t.TrtGroup <> "BRIDGE" AndAlso
            '        t.TrtLVL <= currentToothLevel) OrElse
            '       t.TrtGroup = "CROWNS ON TOOTH" OrElse
            '       t.TrtGroup = "INDIRECT VENEERS" OrElse
            '       t.TrtGroup = "DIRECT VENEERS" OrElse
            '       (t.ParentGroup IsNot Nothing AndAlso t.ParentGroup = "CROWNS ON IMPLANT") Then Continue For
            '    Case 6
            '        If (t.TrtGroup <> "EXTRACTION" AndAlso
            '        t.TrtGroup <> "BRIDGE" AndAlso
            '        t.TrtLVL < currentToothLevel) OrElse
            ...
            '       t.TrtGroup = "INDIRECT VENEERS" OrElse
            '       t.TrtGroup = "DIRECT VENEERS" OrElse
            '       (t.ParentGroup IsNot Nothing AndAlso t.ParentGroup = "IMPLANT") Then Continue For
            '    Case 7
            '        If (t.TrtGroup <> "EXTRACTION" AndAlso
            '        t.TrtGroup <> "BRIDGE" AndAlso
            '        t.TrtLVL <> currentToothLevel) OrElse
            '       t.TrtGroup = "CROWNS ON TOOTH" OrElse
            '       (t.ParentGroup IsNot Nothing AndAlso t.ParentGroup = "IMPLANT") OrElse
            '       t.TrtGroup = "INDIRECT VENEERS" OrElse
            '       t.TrtGroup = "DIRECT VENEERS" OrElse
            '       (t.ParentGroup IsNot Nothing AndAlso t.ParentGroup = "CROWNS ON IMPLANT") Then Continue For
            '    Case 8
            ...
            ' Get shape Inf
            Dim shape As Shapes = allShapes.FirstOrDefault(Function(s) s.ShapeID = t.ShapeID)

            ' **KEY FIX: Split ParentGroup just like in multi-tooth query**
            Dim parentGroups As String()
            If String.IsNullOrEmpty(t.ParentGroup) Then
                parentGroups = {"OTHER TREATMENTS"}
            Else
                parentGroups = t.ParentGroup.Split(","c)
            End If

            ' Create a treatment node for each parent group (just like STRING_SPLIT does)
            For Each ParentGroup In parentGroups
                Dim cleanParentGroup As String = ParentGroup.Trim()
                ...
                .TrtLoc = t.TrtLoc,
                .TrtColor = t.TrtColor,
                .TrtBrdrClr = t.TrtBrdrClr,
                .TrtBrdrThick = t.TrtBrdrThick,
                .ShapeID = t.ShapeID,
                .ShapeName = If(shape Is Nothing, Nothing, shape.ShapeName),
                .TrtDetails = t.TrtDetails,
                .TrtLVL = t.TrtLVL,
                .ToothID = t.ToothID,
                .OldTrt = t.OldTrt,
                .TrtGroup = t.TrtGroup,
                .ParentGroup = cleanParentGroup,  ' Use the split parent group
                .KidTrt = t.KidTrt
            }

                splitTreatments.Add(nodeInfo)
            Next
        Next

        ' ----------------------------------------------------------
        ' STEP 3: Group treatments by TrtGroup (EXACTLY like multi-select)
        ' ----------------------------------------------------------
        Dim treatmentsByGroup As New Dictionary(Of String, List(Of TrtNodeInfo))()

        For Each treatment In splitTreatments
            Dim groupName As String = treatment.TrtGroup

            If Not treatmentsByGroup.ContainsKey(groupName) Then
                treatmentsByGroup.Add(groupName, New List(Of TrtNodeInfo)())
            End If

            treatmentsByGroup(groupName).Add(treatment)
        Next

        ' ----------------------------------------------------------
        ' STEP 4: Build tree using the EXACT same logic as multi-select
        ' ----------------------------------------------------------
        BuildTreeWithMultipleGroupsSingle(treatmentsByGroup, existingTrts)
        ...
        End If
    End Sub
    Private Sub BuildTreeWithMultipleGroupsSingle(treatmentsByGroup As Dictionary(Of String, List(Of TrtNodeInfo)), existingTrts As HashSet(Of Integer))
        TrtsTreeView.Nodes.Clear()
        ' Create dictionaries to organize nodes
        Dim parentGroupNodes As New Dictionary(Of String, TreeNode)()
        Dim trtGroupNodes As New Dictionary(Of String, TreeNode)()
        ...
        For Each groupEntry In treatmentsByGroup
            Dim treatmentsList As List(Of TrtNodeInfo) = groupEntry.Value
            For Each treatment In treatmentsList
                Dim parentGroupName As String = treatment.ParentGroup
                ' Create parent group node if it doesn't exist
                If Not parentGroupNodes.ContainsKey(parentGroupName) Then
                    Dim parentNode As New TreeNode(parentGroupName)
                    parentNode.Tag = "PARENT_GROUP"
        ...
        ' Third pass: Add treatments to their groups - USE EXACT SAME LOGIC AS FIRST SUB
        For Each groupEntry In treatmentsByGroup
            Dim groupName As String = groupEntry.Key
            Dim treatmentsList As List(Of TrtNodeInfo) = groupEntry.Value
            For Each treatment In treatmentsList
                ...
                Dim groupKey = $"{parentGroupName}|{groupName}"
                If trtGroupNodes.ContainsKey(groupKey) Then
                    ' USE THE EXACT SAME TREATMENT TEXT LOGIC AS FIRST SUB
                    Dim trtstr As String = ""
                    If parentGroupName.Contains("DIAGNOSIS") Then
                        trtstr = $"{treatment.Trt} - {parentGroupName}"
                    Else
                        trtstr = $"{treatment.Trt}"  ' Just the treatment name, no parent group
                    End If
                    Dim treatmentNode As New TreeNode(trtstr)
                    treatmentNode.Tag = treatment
                    ' USE THE EXACT SAME COLOR LOGIC AS FIRST SUB
                    treatmentNode.ForeColor = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)), Color.Red, Color.Black)
                    ' USE THE EXACT SAME FONT LOGIC AS FIRST SUB
                    treatmentNode.NodeFont = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)),
                            New Font(TrtsTreeView.Font, FontStyle.Strikeout), TrtsTreeView.Font)
                    treatmentNode.Name = treatment.TrtID.ToString()
                    ' Optional: Add treatment details as tooltip
        ...
        ' Auto-expand parent groups that have only one child for better visibility (FROM FIRST SUB)
        For Each parentNode In parentGroupNodes.Values
            If parentNode.Nodes.Count = 1 Then
                parentNode.Expand()
            End If
    ...
    Private Sub BuildTreeWithMultipleGroupsSingle1(treatmentsByGroup As Dictionary(Of String, List(Of TrtNodeInfo)), existingTrts As HashSet(Of Integer))
        TrtsTreeView.Nodes.Clear()
        ' Create dictionaries to organize nodes
        Dim parentGroupNodes As New Dictionary(Of String, TreeNode)()
        Dim trtGroupNodes As New Dictionary(Of String, TreeNode)()
        ' First pass: Create all parent group nodes by splitting comma-separated ParentGroup
        For Each groupEntry In treatmentsByGroup
            Dim treatmentsList As List(Of TrtNodeInfo) = groupEntry.Value
            For Each treatment In treatmentsList
                ' Split comma-separated ParentGroup into individual parent groups
                Dim parentGroups As String() = treatment.ParentGroup.Split(","c)
                For Each parentGroupName In parentGroups
                    Dim cleanParentGroup As String = parentGroupName.Trim()
                    ' Create parent group node if it doesn't exist
                    If Not String.IsNullOrEmpty(cleanParentGroup) AndAlso Not parentGroupNodes.ContainsKey(cleanParentGroup) Then
                        Dim parentNode As New TreeNode(cleanParentGroup)
                        parentNode.Tag = "PARENT_GROUP"
                        parentNode.ForeColor = Color.DarkBlue
                        parentNode.NodeFont = New Font(TrtsTreeView.Font, FontStyle.Bold)
                        parentGroupNodes(cleanParentGroup) = parentNode
                        TrtsTreeView.Nodes.Add(parentNode)
                    End If
                Next
            Next
        ...
        For Each groupEntry In treatmentsByGroup
            ...
            For Each treatment In treatmentsList
                ...
                For Each parentGroupName In parentGroups
                    Dim cleanParentGroup As String = parentGroupName.Trim()
                    Dim groupKey = $"{cleanParentGroup}|{groupName}"
                    ' Create treatment group node if it doesn't exist under this parent
                    If parentGroupNodes.ContainsKey(cleanParentGroup) AndAlso Not trtGroupNodes.ContainsKey(groupKey) Then
                        Dim groupNode As New TreeNode(groupName)
                        groupNode.Tag = "TREATMENT_GROUP"
                        groupNode.ForeColor = Color.DarkGreen
                        trtGroupNodes(groupKey) = groupNode
                        parentGroupNodes(cleanParentGroup).Nodes.Add(groupNode)
                    End If
                Next
            Next
        Next
        ...
        For Each groupEntry In treatmentsByGroup
            ...
            For Each treatment In treatmentsList
                ...
                For Each parentGroupName In parentGroups
                    ...
                    Dim groupKey = $"{cleanParentGroup}|{groupName}"
                    If trtGroupNodes.ContainsKey(groupKey) Then
                        'Dim treatmentNode As New TreeNode($"{treatment.Trt} - {cleanParentGroup}")
                        Dim trtstr As String = ""
                        If cleanParentGroup.Contains("DIAGNOSIS") Then
                            trtstr = $"{treatment.Trt} - {cleanParentGroup}"
                        Else
                            trtstr = $"{treatment.Trt}"
                        End If
                        Dim treatmentNode As New TreeNode(trtstr) '
                        treatmentNode.Tag = treatment
                        treatmentNode.ForeColor = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)), Color.BlueViolet, Color.Blue)
                        treatmentNode.NodeFont = If(existingTrts.Contains(Convert.ToInt32(treatment.TrtID)),
                               New Font(TrtsTreeView.Font, FontStyle.Strikeout), TrtsTreeView.Font)
                        treatmentNode.Name = treatment.TrtID.ToString()
                        ' Optional: Add treatment details as tooltip
                        treatmentNode.ToolTipText = $"Value: {treatment.TrtValue} | Location: {treatment.TrtLoc}"
                        ' Only add the treatment if it hasn't been added to this group yet
                        If Not trtGroupNodes(groupKey).Nodes.ContainsKey(treatment.TrtID.ToString()) Then
                            trtGroupNodes(groupKey).Nodes.Add(treatmentNode)
                        End If
                    End If
                Next
    ...
    Private Sub BuildTreeFromList(nodeList As List(Of TrtNodeInfo), Optional existingTrts As HashSet(Of Integer) = Nothing)
        ...
        Try
            ...
            For Each Inf In nodeList
                ...
                If Not String.IsNullOrEmpty(parent) Then
                    If Not groupNodes.ContainsKey(parent) Then
                        Dim pnode As New TreeNode(parent) With {
                        .Name = parent,
                        .ForeColor = Color.DarkBlue,
                        .Tag = "PARENT_GROUP"
                    }
                        groupNodes.Add(parent, pnode)
                    End If
                End If
            ...
            ' Add all top-level nodes (those without Parent) to TreeView
            For Each kvp In groupNodes
                If kvp.Value.Parent Is Nothing Then
                    TrtsTreeView.Nodes.Add(kvp.Value)
                End If
...
#Region "New Mob TreeViewSource"
    ...
    Private Sub SetMobTreeSnglTooth(toothID As String, toothNum As Byte)
        ...
        If TrtSourceHelper.PatientHasDentureOnAnyTooth(PatientID, New Integer() {CInt(toothNum)}) Then
            AllTrtNodes.Clear()
            Dim empty As New List(Of TrtSourceHelper.TrtNodeInfo)()
            TrtSourceHelper.BuildTreeFromList(TrtsTreeView, empty, Nothing, "No treatments available: this tooth has a denture.")
            SaveFullTreeSnapshot()
            Return
        End If
...
#Region "eNTER KEY AND dOUBLE CLICK AddedTrts AND TreeView Events"
    ...
    Private Sub AddedTrtsList_KeyDown(sender As Object, e As KeyEventArgs)
        ...
        End If
    End Sub
    Private Sub AddedTrtsList_MouseDoubleClick(sender As Object, e As MouseEventArgs)
        ' Ensure an item is selected
        If AddedTrtsList.SelectedItem Is Nothing Then
            MessageBox.Show("Please select an item from the list.", "No Selection", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
    ...
    End Sub
    Private Sub TrtsTreeView_KeyDown(sender As Object, e As KeyEventArgs)
...
#Region "TreeView Helpers"
    ...
    Private Sub TrtsTreeView_AfterSelect(sender As Object, e As TreeViewEventArgs)
    ...
    End Class
    'TrtsTreeView
    Private Sub TrtsTreeView_MouseDown(sender As Object, e As MouseEventArgs)
        ...
        End If
    End Sub
    Private Sub TrtsTreeView_MouseMove(sender As Object, e As MouseEventArgs)
    ...
    Private Function BuildToothTrt(svg As SvgImageBox, node As TreeNode, treatmentText As String) As Patient_ToothTrt
        ...
        .PropertyName = node.Tag.ShapeName,
...
#Region "Loading"
    ...
    Private Sub AdultJawNewCode_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        End If
        'sw.Stop()
        'LogToFile("AdultJawNewCode_Load Time spent: " & sw.Elapsed.TotalSeconds.ToString("F2") & " seconds", Me)
        LogTime(NameOf(AdultJawNewCode_Load), Me.Name, sw)
        ' Subscribe to the event from the main form
        'Dim baseForm = FormManager.Instance.CurrentForm ' BasePatientWorkspace
        'If baseForm IsNot Nothing AndAlso baseForm.HeaderControl IsNot Nothing Then
        '    AddHandler baseForm.HeaderControl.KidStatusChanged, AddressOf OnKidStatusChanged
        'End If

    End Sub

    Private Sub OnKidStatusChanged(kidText As String, isKid As Boolean, isManual As Boolean)
        ' Update your UserControl based on the new status
        ' You can use kidText, isKid, isManual as needed
        If kidText IsNot Nothing AndAlso (kidText.Contains("(Manual)") OrElse kidText.Contains("(يدوي)")) Then
            horSep.LineColor = Color.Red
            vertSep.LineColor = Color.Red
            horSep.LineStyle = DashStyle.DashDot
            vertSep.LineStyle = DashStyle.DashDot
    ...
    Private Const OriginalPanelHeight As Integer = 643 ' 654
    ...
    Private Sub AdultJawNewCode_Resize(sender As Object, e As EventArgs) Handles Me.Resize
        If controlBoundsCache.IsEmpty Then Return
        ResizeControlsProportionally()
        ALIGNS()
    End Sub
...
#Region "Aligning"
    ...
    Private Sub AdultJawNewCode_Disposed(sender As Object, e As EventArgs) Handles Me.Disposed
        DetachJawHandlers()
        Try
            zoomTimer.Stop()
            zoomTimer.Dispose()
