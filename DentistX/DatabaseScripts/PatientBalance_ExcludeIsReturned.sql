...
-- TotalTreatments = SUM(TrtValue - Discount - Discount2) on Patient_Trts (matches GetPatientBalance).
...
BEGIN
    ;WITH TreatmentTotal AS (
             SELECT [PatientID],
                    SUM(ISNULL([TrtValue], 0) - ISNULL([Discount], 0) - ISNULL([Discount2], 0)) AS TotalTreatments
