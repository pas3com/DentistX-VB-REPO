...
Public Class Patient_DiagnosisDATA
    ...
    ''' <summary>True if this tooth has at least one LVL 4 (extraction) diagnosis row.</summary>
    Public Function HasExtractionLevel(patientID As Integer, toothNum As Byte) As Boolean
        Const sql As String =
            "SELECT CASE WHEN EXISTS (SELECT 1 FROM dbo.Patient_Diagnosis WHERE PatientID = @PatientID AND ToothNum = @ToothNum AND LVL = 4) " &
            "THEN 1 ELSE 0 END"
        Using connection As New SqlConnection(ConnectionString)
