...
Public Class Patient_MobileDiagDATA
    Implements IDisposable

    Private ConnectionString As String = DentistXDATA.GetConnection.ConnectionString


    Public Function GetPatient_TrtScope(ByVal clsPatient_Mobile As Patient_MobileDiag) As IEnumerable(Of Patient_TrtScope)
    ...
    Public Function GetPatient_Trts(ByVal clsPatient_Mobile As Patient_MobileDiag) As IEnumerable(Of Patient_Trts)
    ...
    Public Function SelectAll() As IEnumerable(Of Patient_MobileDiag)

        Using conn As New SqlConnection(ConnectionString)
            conn.Open()
            Return conn.Query(Of Patient_MobileDiag)("SELECT * FROM Patient_MobileDiag  WHERE  MobID=@MobID AND PatientID = @PatientID AND ToothName = @ToothNam")
        End Using
    End Function

    Public Function SelectByID(ByVal clsPatient_Mobile As Patient_MobileDiag) As Patient_MobileDiag
        Using conn As New SqlConnection(ConnectionString)
            Dim sql As String = "Select * FROM  Patient_MobileDiag  WHERE MobID=@MobID "
            Return conn.QuerySingleOrDefault(Of Patient_MobileDiag)(sql, New With {.MobID = clsPatient_Mobile.MobID})
        End Using
        'AppLogger.Log(CurrentUser.UsName, PatientName, "Patient_ToothTrt", "SELECT")
    End Function
    Public Function Select_Record(ByVal clsPatient_Mobile As Patient_MobileDiag, Optional ByVal trans As SqlTransaction = Nothing) As Patient_MobileDiag
        If trans IsNot Nothing Then
            Dim sql As String = "Select * FROM Patient_MobileDiag WHERE MobID = @MobID AND PatientID = @PatientID AND ToothName = @ToothName"
            Return trans.Connection.QuerySingleOrDefault(Of Patient_MobileDiag)(
            sql,
           New With {
        ...
        Else
            ...
            Using conn As New SqlConnection(ConnectionString)
                Dim sql As String = "Select * FROM Patient_MobileDiag WHERE MobID = @MobID AND PatientID = @PatientID AND ToothName = @ToothName"
                Return conn.QuerySingleOrDefault(Of Patient_MobileDiag)(
                sql,
                New With {
               .MobID = clsPatient_Mobile.MobID,
               .PatientID = clsPatient_Mobile.PatientID,
    ...
    Public Function Select_RecordByTrtIDByTNum(ByVal clsPatient_Mobile As Patient_MobileDiag) As Patient_MobileDiag
        Using conn As New SqlConnection(ConnectionString)
            'Dim sql As String = "Select * FROM Patient_MobileDiag WHERE TrtID = @TrtID And PatientID = @PatientID And ToothNum = @ToothNum"
            Dim sql As String = "Select * FROM Patient_MobileDiag WHERE MobID=@MobID And  PatientID = @PatientID And ToothNum = @ToothNum"
            'Return conn.QuerySingleOrDefault(Of Patient_MobileDiag)(sql, New With {.TrtID = clsPatient_Mobile.TrtID, .PatientID = clsPatient_Mobile.PatientID, .ToothNum = clsPatient_Mobile.ToothNum})
            Return conn.QuerySingleOrDefault(Of Patient_MobileDiag)(sql, New With {.MobID = clsPatient_Mobile.MobID,
                                                                                 .PatientID = clsPatient_Mobile.PatientID,
                                                                                 .ToothNum = clsPatient_Mobile.ToothNum})

        End Using
        AppLogger.Log(CurrentUser.UsName, PatientName, "Patient_MobileDiag", "SELECT")
    End Function

    Public Function Select_BypID_tNum_prpName(ByVal clsPatient_Mobile As Patient_MobileDiag) As Patient_MobileDiag
        Using conn As New SqlConnection(ConnectionString)
            'Dim sql As String = "Select * FROM Patient_MobileDiag WHERE TrtID = @TrtID And PatientID = @PatientID And ToothNum = @ToothNum"
            Dim sql As String = "Select * FROM Patient_MobileDiag WHERE  PatientID = @PatientID And ToothNum = @ToothNum And PropertyName=@PropertyName"
            'Return conn.QuerySingleOrDefault(Of Patient_MobileDiag)(sql, New With {.TrtID = clsPatient_Mobile.TrtID, .PatientID = clsPatient_Mobile.PatientID, .ToothNum = clsPatient_Mobile.ToothNum})
            Return conn.QuerySingleOrDefault(Of Patient_MobileDiag)(sql, New With {.PatientID = clsPatient_Mobile.PatientID,
                                                                                 .ToothNum = clsPatient_Mobile.ToothNum,
    ...
    Public Function Select_BypID_tNum(ByVal clsPatient_Mobile As Patient_MobileDiag) As IEnumerable(Of Patient_MobileDiag)
        Using conn As New SqlConnection(ConnectionString)
            'Dim sql As String = "Select * FROM Patient_MobileDiag WHERE TrtID = @TrtID And PatientID = @PatientID And ToothNum = @ToothNum"
            Dim sql As String = "Select * FROM Patient_MobileDiag WHERE  PatientID = @PatientID And ToothNum = @ToothNum"
            'Return conn.QuerySingleOrDefault(Of Patient_MobileDiag)(sql, New With {.TrtID = clsPatient_Mobile.TrtID, .PatientID = clsPatient_Mobile.PatientID, .ToothNum = clsPatient_Mobile.ToothNum})
            Return conn.Query(Of Patient_MobileDiag)(sql, New With {.PatientID = clsPatient_Mobile.PatientID,
                                                                                 .ToothNum = clsPatient_Mobile.ToothNum})
    ...
    Public Function GetPatientMobile(ByVal PatientID As Integer) As IEnumerable(Of Patient_MobileDiag)
        Using conn As New SqlConnection(ConnectionString)
            Dim sql As String = "Select dbo.Patient_MobileDiag.MobID, dbo.Patient_MobileDiag.PatientID, dbo.Patient_MobileDiag.ToothNum,
                             dbo.Patient_MobileDiag.ToothName, dbo.Patient_MobileDiag.LVL, dbo.Patient_MobileDiag.PropertyName, dbo.Patient_MobileDiag.FillColor,
                             dbo.Patient_MobileDiag.BorderThickness, dbo.Patient_MobileDiag.BorderColor, dbo.Patient_MobileDiag.TreatmentType,
                             dbo.Patient_MobileDiag.TreatDate, dbo.Patient_MobileDiag.Treat, dbo.Patient_MobileDiag.TreatPlan,
                             dbo.Patient_MobileDiag.TreatDetails, dbo.Patient_MobileDiag.TreatNotes, dbo.Patient_MobileDiag.Finished,
                             dbo.Patient_MobileDiag.TreatEndDate, dbo.Patient_MobileDiag.IsExternal, dbo.Patient_MobileDiag.ExternalClinicName,
                             dbo.Patient_MobileDiag.ExternalTreatmentDate, dbo.Patient_MobileDiag.IsPaid, dbo.Shapes.ShapeID, dbo.Shapes.ShapeName, dbo.Shapes.ShapeDetail,
                             dbo.Shapes.OutID, dbo.Shapes.TopID, dbo.Shapes.INID
                             From dbo.Patient_MobileDiag INNER Join
                             dbo.Shapes ON dbo.Patient_MobileDiag.ShapeID = dbo.Shapes.ShapeID
                             Where PatientID = @PatientID"
            Return conn.Query(Of Patient_MobileDiag)(sql, New With {.PatientId = PatientID})
        End Using
        AppLogger.Log(CurrentUser.UsName, PatientName, "Patient_MobileDiag", "SELECT")
    End Function
    Public Function GetPatientToothMobiles(ByVal PatientID As Integer, ByVal ToothNum As Integer) As IEnumerable(Of Patient_MobileDiag)
        Using conn As New SqlConnection(ConnectionString)
            Dim sql As String = "Select dbo.Patient_MobileDiag.MobID, dbo.Patient_MobileDiag.PatientID, dbo.Patient_MobileDiag.ToothNum,
                             dbo.Patient_MobileDiag.ToothName, dbo.Patient_MobileDiag.LVL, dbo.Patient_MobileDiag.PropertyName, dbo.Patient_MobileDiag.FillColor,
                             dbo.Patient_MobileDiag.BorderThickness, dbo.Patient_MobileDiag.BorderColor, dbo.Patient_MobileDiag.TreatmentType,
                             ...
                             dbo.Shapes ON dbo.Patient_MobileDiag.ShapeID = dbo.Shapes.ShapeID
                            Where PatientID = @PatientID AND ToothNum=@ToothNum"
            Return conn.Query(Of Patient_MobileDiag)(sql, New With {.PatientId = PatientID, .ToothNum = ToothNum})
        End Using
    End Function



    Public Function GetPatientMobiles(patientID As Integer, toothNumList As List(Of Byte)) As IEnumerable(Of Patient_MobileDiag)
        ...
        Dim query = "SELECT * FROM [Patient_MobileDiag] 
                WHERE PatientID = @PatientID 
        ...
        Using connection As New SqlConnection(ConnectionString)
            Try
                ...
                Return Conn.Query(Of Patient_MobileDiag)(
                query,
                New With {
                    .PatientID = patientID,
    ...
    Public Function AddTransactional(conn As SqlConnection, trans As SqlTransaction, clsPatient_Mobile As Patient_MobileDiag) As Boolean
        Dim query As String = "
MERGE INTO [dbo].[Patient_MobileDiag] AS Target
USING (
    SELECT 
...
"
    ...
    Public Function Add(ByVal clsPatient_Mobile As Patient_MobileDiag) As Boolean
        Dim RowsAffected As Integer = 0
        Using conn As New SqlConnection(ConnectionString)
            Dim sql As String = "INSERT INTO Patient_MobileDiag (PatientID, ShapeID, ToothNum, ToothName, LVL, PropertyName, FillColor, BorderThickness,
    ...
    Public Function Update1(oldPatient_Mobile As Patient_MobileDiag, newPatient_Mobile As Patient_MobileDiag) As Boolean
        Using conn As New SqlConnection(ConnectionString)
            ...
            Dim affectedRows As Integer = conn.Execute("UPDATE [Patient_MobileDiag] SET [PatientID] = @NewPatientID, [ShapeID] = @NewShapeID, [ToothNum] = @NewToothNum,
    ...
    Public Function Update(oldPatient_Mobile As Patient_MobileDiag, newPatient_Mobile As Patient_MobileDiag) As Boolean
        Using conn As New SqlConnection(ConnectionString)
            Dim parameters = New With {
            ...
            Dim query As String = "UPDATE [Patient_MobileDiag] SET 
        ...
        End Using
    End Function
    Public Function Delete(ByVal clsPatient_Mobile As Patient_MobileDiag) As Boolean
        Dim deleteStatement As String =
            "DELETE FROM [Patient_MobileDiag] 
			WHERE MobID = @MobID"
    ...
    Public Function DeleteTrans(ByVal clsPatient_Mobile As Patient_MobileDiag) As Boolean
    ...
    BEGIN TRY
        ...
        INSERT INTO @MobIDs
        SELECT MobID FROM [Patient_MobileDiag]
        WHERE PatientID = @PatientID 
        AND ToothNum IN (SELECT value FROM STRING_SPLIT(@ToothNumList, ','));
        ...
        -- 3. Finally delete from [Patient_MobileDiag]
        DELETE FROM [Patient_MobileDiag]
        WHERE PatientID = @PatientID 
        AND ToothNum IN (SELECT value FROM STRING_SPLIT(@ToothNumList, ','));
    ...
    "
        ...
        Using connection As New SqlConnection(ConnectionString)
            ...
            Try
                ...
                If result > 0 Then
                    'AppLogger.Log(CurrentUser.UsName, clsPatient.PatientName, "[Patient_MobileDiag]",
    ...
    Public Function DeleteTrans1(ByVal clsPatient_Mobile As Patient_MobileDiag) As Boolean
        Dim deleteStatement As String = "
        ...
        IF EXISTS (SELECT 1 FROM [Patient_MobileDiag] 
    ...
    Public Function SyncExtractionsAndImplantsToMobile() As Integer
        Using conn As New SqlConnection(ConnectionString)
            ' Query to insert only new records that don't already exist in Patient_MobileDiag
            Dim query As String = "
        INSERT INTO [dbo].[Patient_MobileDiag]
           ([PatientID]
           ,[ShapeID]
           ,[ToothNum]
           ,[ToothName]
        ...
        WHERE 
            ...
            AND NOT EXISTS (
                SELECT 1 
                FROM [dbo].[Patient_MobileDiag] PM 
                WHERE 
    ...
    Public Function SyncExtractionsAndImplantsToMobile1() As Integer
        ...
        SELECT 
            ...
            PT.[ExternalTreatmentDate],
            PT.[IsPaid]
        FROM [dbo].[Patient_MobileDiag] PT
        LEFT JOIN [dbo].[Patient_MobileDiag] PM ON 
