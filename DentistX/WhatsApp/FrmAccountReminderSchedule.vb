...
''' <summary>Manage recurring account reminders: select patients and set reminder interval (e.g. every 3 days).</summary>
Public Class FrmAccountReminderSchedule
    ...
    Private Sub FrmAccountReminderSchedule_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    ''' </summary>
    Private Shared Function ValidateWhatsAppNumber(phone As String) As String
        Return WhatsHelper.ValidateWhatsAppNumberLocalized(phone)
    End Function

    ''' <summary>Normalize to digits only (shared helper).</summary>
    Private Shared Function NormalizeWhatsAppNumber(number As String) As String
        Return WhatsHelper.NormalizeWhatsDigits(number)
    End Function
    ...
    Private Sub ShowWhatsAppPanel(patientName As String, currentNumber As String)
        ...
        TxtWhatsApp.Text = NormalizeWhatsAppNumber(currentNumber)
    ...
    Private Sub LoadSchedule()
        Dim schedule = AccountReminderScheduleService.GetAllScheduled()
        _rows.Clear()
    ...
    Private Function ResolvePatientForAdd() As PatientItem
        ...
        If pid <= 0 Then
            ...
            If String.IsNullOrEmpty(txt) Then
                ...
                Return Nothing
            End If
            Dim match = _patientData.SelectAll().FirstOrDefault(Function(p) String.Equals(p.PatientName, txt, StringComparison.OrdinalIgnoreCase))
            If match Is Nothing Then match = _patientData.SelectAll().FirstOrDefault(Function(p) (If(p.PatientName, "")).Contains(txt))
    ...
    Private Sub BtnSaveWhatsApp_Click(sender As Object, e As EventArgs)
        Dim num = NormalizeWhatsAppNumber(If(TxtWhatsApp.Text, ""))
        Dim validationMsg As String = ValidateWhatsAppNumber(num)
        If validationMsg <> "" Then
    ...
    Private Sub DoAddToSchedule(sel As PatientItem, days As Integer)
        ...
        End If
        AccountReminderScheduleService.AddOrUpdate(sel.PatientID, days, useEng)
        LoadSchedule()
    ...
    Private Sub BtnRemove_Click(sender As Object, e As EventArgs)
        ...
        AccountReminderScheduleService.Remove(row.PatientID)
        LoadSchedule()
    End Sub
    ...
    Private Sub ViewSchedule_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles ViewSchedule.CellValueChanged
        ...
        If Integer.TryParse(If(e.Value?.ToString(), "0"), days) AndAlso days > 0 Then
            AccountReminderScheduleService.AddOrUpdate(row.PatientID, days, row.MessageEnglish)
