...
Partial Public Class FrmImpClrs
 ...
 End Sub
 Private clsImpClrsData As New ImpClrsData
 Private clsDentistXData As New DentistXData
 
 
 Private Sub FrmImpClrs_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
     LoadDGV()
 ...
 Private Sub LoadDGV()
     Try
         With DGV
             .DataSource = clsImpClrsData.SelectAll
 ...
 Private Sub Display()
     ClearRecord()
     Dim clsImpClrs As New ImpClrs
        clsImpClrs.ImpClrID = System.Convert.ToInt32(dgView.GetRowCellDisplayText(Row, colImpClrID))
    clsImpClrs = clsImpClrsData.Select_Record(clsImpClrs)
     If Not clsImpClrs Is Nothing Then
     Try
        ImpClrIDSpinEdit.Value = System.Convert.ToInt32(clsImpClrs.ImpClrID)
        ImpClrTextEdit.Text = Convert.ToString(clsImpClrs.ImpClr)
     Catch ex As Exception
          MsgBox(ex.Message)
     End Try
     End If
 ...
 Private Sub Add()
     ...
     ImpClrIDSpinEdit.Select
        ImpClrIDSpinEdit.Text = DentistXDATA.getAutoID("New", "ImpClrs")
    End Sub
 ...
 Private Sub ClearRecord()
    ImpClrIDSpinEdit.Value = 0
    ImpClrTextEdit.Text = Nothing
 End Sub
 ...
 End Sub
 Private Sub SetData(ByVal clsImpClrs As ImpClrs)
     With clsImpClrs
        .ImpClrID = System.Convert.ToInt32(ImpClrIDSpinEdit.Value)
        .ImpClr = System.Convert.ToString(ImpClrTextEdit.Text)
     End With
 End Sub
    ...
    Private Sub InsertRecord()
        ...
        If VerifyData() = True Then
            ...
            bSuccess = clsImpClrsData.Add(clsImpClrs)
    ...
    Private Sub UpdateRecord()
        Dim oclsImpClrs As New ImpClrs
        Dim clsImpClrs As New ImpClrs
        oclsImpClrs.ImpClrID = System.Convert.ToInt32(dgView.GetRowCellDisplayText(Row, colImpClrID))
        oclsImpClrs = clsImpClrsData.Select_Record(oclsImpClrs)
        If VerifyData() = True Then
            SetData(clsImpClrs)
            Dim bSuccess As Boolean
            bSuccess = clsImpClrsData.Update(oclsImpClrs, clsImpClrs)
            If bSuccess = True Then
                GoBack_To_Grid()
            Else
    ...
    Private Sub DeleteRecord()
        ...
        If result = DialogResult.Yes Then
            Dim bSuccess As Boolean
            bSuccess = clsImpClrsData.Delete(clsImpClrs)
            If bSuccess = True Then
                GoBack_To_Grid()
            Else
    ...
    Private Function VerifyData() As Boolean
        ...
        If ImpClrIDSpinEdit.Value = Nothing OrElse ImpClrIDSpinEdit.Value.ToString() = "" Then
            MsgBox("ImpClrID is required.", MsgBoxStyle.OkOnly, "Entry Error")
            ImpClrIDSpinEdit.Select()
            Return False
        End If
        If ImpClrTextEdit.Text = "" Then
            MsgBox("ImpClr is required.", MsgBoxStyle.OkOnly, "Entry Error")
            ImpClrTextEdit.Select()
            Return False
        End If
        Return True
    End Function
