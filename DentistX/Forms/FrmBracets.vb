...
Partial Public Class FrmBracets
 ...
 End Sub
 Private clsBracetsData As New BracetsData
 Private clsDentistXData As New DentistXData
 
 
 Private Sub FrmBracets_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
     LoadDGV()
 ...
 Private Sub LoadDGV()
     Try
         With DGV
             .DataSource = clsBracetsData.SelectAll
 ...
 Private Sub EnableRecord(ByVal YesNo As Boolean)
        BracetIDSpinEdit.Enabled = False
        BracetNameTextEdit.Enabled = YesNo
 End Sub
 ...
 Private Sub Display()
     ClearRecord()
     Dim clsBracets As New Bracets
     clsBracets.BracetID = System.Convert.ToInt32(dgView.GetRowCellDisplayText(Row, colBracetID))
     clsBracets = clsBracetsData.Select_Record(clsBracets.BracetID)
     If Not clsBracets Is Nothing Then
     Try
        BracetIDSpinEdit.Value = System.Convert.ToInt32(clsBracets.BracetID)
        BracetNameTextEdit.Text = Convert.ToString(clsBracets.BracetName)
     Catch ex As Exception
          MsgBox(ex.Message)
     End Try
     End If
 ...
 Private Sub ClearRecord()
    BracetIDSpinEdit.Value = 0
    BracetNameTextEdit.Text = Nothing
 End Sub
 ...
 End Sub
 Private Sub SetData(ByVal clsBracets As Bracets)
     With clsBracets
        .BracetID = System.Convert.ToInt32(BracetIDSpinEdit.Value)
        .BracetName = System.Convert.ToString(BracetNameTextEdit.Text)
     End With
 End Sub
    ...
    Private Sub InsertRecord()
        ...
        If VerifyData() = True Then
            ...
            bSuccess = clsBracetsData.Add(clsBracets)
    ...
    Private Sub UpdateRecord()
        Dim oclsBracets As New Bracets
        Dim clsBracets As New Bracets
        oclsBracets.BracetID = System.Convert.ToInt32(dgView.GetRowCellDisplayText(Row, colBracetID))
        oclsBracets = clsBracetsData.Select_Record(oclsBracets.BracetID)
        If VerifyData() = True Then
            SetData(clsBracets)
            Dim bSuccess As Boolean
            bSuccess = clsBracetsData.Update(oclsBracets, clsBracets)
            If bSuccess = True Then
                GoBack_To_Grid()
            Else
    ...
    Private Sub DeleteRecord()
        ...
        If result = DialogResult.Yes Then
            Dim bSuccess As Boolean
            bSuccess = clsBracetsData.Delete(clsBracets)
            If bSuccess = True Then
                GoBack_To_Grid()
            Else
    ...
    Private Function VerifyData() As Boolean
        ...
        If BracetIDSpinEdit.Value = Nothing OrElse BracetIDSpinEdit.Value.ToString() = "" Then
            MsgBox("BracetID is required.", MsgBoxStyle.OkOnly, "Entry Error")
            BracetIDSpinEdit.Select()
            Return False
        End If
        If BracetNameTextEdit.Text = "" Then
            MsgBox("BracetName is required.", MsgBoxStyle.OkOnly, "Entry Error")
            BracetNameTextEdit.Select()
            Return False
        End If
        Return True
    End Function
