...
Partial Public Class FrmGroups
 ...
 End Sub
 Private clsGroupsData As New GroupsData
 Private clsDentistXData As New DentistXData
 
 
 Private Sub FrmGroups_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
     LoadDGV()
 ...
 Private Sub LoadDGV()
     Try
         With DGV
             .DataSource = clsGroupsData.SelectAll
 ...
 Private Sub EnableRecord(ByVal YesNo As Boolean)
        GroupNameTextEdit.Enabled = YesNo
    End Sub
 ...
 Private Sub Display()
     ClearRecord()
     Dim clsGroups As New Groups
        clsGroups.GroupID = System.Convert.ToInt32(dgView.GetRowCellDisplayText(Row, colGroupID))
    clsGroups = clsGroupsData.Select_Record(clsGroups)
     If Not clsGroups Is Nothing Then
     Try
        GroupIDSpinEdit.Value = System.Convert.ToInt32(clsGroups.GroupID)
        GroupNameTextEdit.Text = Convert.ToString(clsGroups.GroupName)
     Catch ex As Exception
          MsgBox(ex.Message)
     End Try
     End If
 ...
 Private Sub ClearRecord()
    GroupIDSpinEdit.Value = 0
    GroupNameTextEdit.Text = Nothing
 End Sub
 ...
 End Sub
 Private Sub SetData(ByVal clsGroups As Groups)
     With clsGroups
        .GroupID = System.Convert.ToInt32(GroupIDSpinEdit.Value)
        .GroupName = System.Convert.ToString(GroupNameTextEdit.Text)
     End With
 End Sub
    ...
    Private Sub InsertRecord()
        ...
        If VerifyData() = True Then
            ...
            bSuccess = clsGroupsData.Add(clsGroups)
    ...
    Private Sub UpdateRecord()
        Dim oclsGroups As New Groups
        Dim clsGroups As New Groups
        oclsGroups.GroupID = System.Convert.ToInt32(dgView.GetRowCellDisplayText(Row, colGroupID))
        oclsGroups = clsGroupsData.Select_Record(oclsGroups)
        If VerifyData() = True Then
            SetData(clsGroups)
            Dim bSuccess As Boolean
            bSuccess = clsGroupsData.Update(oclsGroups, clsGroups)
            If bSuccess = True Then
                GoBack_To_Grid()
            Else
    ...
    Private Sub DeleteRecord()
        ...
        If result = DialogResult.Yes Then
            Dim bSuccess As Boolean
            bSuccess = clsGroupsData.Delete(clsGroups)
            If bSuccess = True Then
                GoBack_To_Grid()
            Else
    ...
    Private Function VerifyData() As Boolean
        ...
        If GroupIDSpinEdit.Value = Nothing OrElse GroupIDSpinEdit.Value.ToString() = "" Then
            MsgBox("GroupID is required.", MsgBoxStyle.OkOnly, "Entry Error")
            GroupIDSpinEdit.Select()
            Return False
        End If
        If GroupNameTextEdit.Text = "" Then
            MsgBox("GroupName is required.", MsgBoxStyle.OkOnly, "Entry Error")
            GroupNameTextEdit.Select()
            Return False
        End If
        Return True
    End Function
